...
function myMAQlibrary(dWagon, nodeData, liftFormatters, tragicPragic) {
    ...
    // for (dIterator = 1; dIterator < consumeData.table.rows.length; dIterator++) {
    //     liftConfig.series[0].data.push(consumeData.table.rows[dIterator][0]);
    //     liftConfig.series[1].data.push(consumeData.table.rows[dIterator][1]);
    //     liftConfig.xAxis.labels.series.push(consumeData.table.rows[dIterator][2]);
    // }
        ...
        MAQ.createLiftChart = function(chartConfigOptions) {
            ...
            if (window.redrawMarker == true) {
                ...
                if (window.blolblolXValue == undefined || window.blolXValue == undefined) {
                    oRectAttr.x = window.abc;
                    if (window.abc == undefined) {
                        oRectAttr.x = 0;
                    }
                } else {
                    widthManger = (window.abc * Math.max(document.documentElement.clientWidth, window.innerWidth));
