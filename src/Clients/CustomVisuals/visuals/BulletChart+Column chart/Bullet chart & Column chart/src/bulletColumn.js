...
// JSHINT count: 0 warnings, 30 Nov,2016
function renderMAQVisual(objDataView, formatOption) {
    ...
    var agreeConfig = {
        ...
        "xAxis": {
            ...
            "labels": {
                ...
                "x": -8 * Math.max(document.documentElement.clientWidth, window.innerWidth || 0) / 200,
                "y": 0
            },
            "numberOfGridLines": 8,
            "gridLineWidth": 0,
    ...
    for (var i = 1; i <= 5; i++) {
        ...
        if (roles[parRecord].length > 0) {
            maxParent = i;
            agreeConfig.series[parentMapper + i].name = objDataView.table.identityFields[roles[parRecord][0]].ref;
        ...
        if (roles[chiRecord].length > 0 && roles[parRecord].length > 0) {
            agreeConfig.series[childMapper + i].name = objDataView.table.identityFields[roles[chiRecord][0]].ref;
    ...
    for (xAxisNames = 0; xAxisNames < insertData.tree.root.children.length; xAxisNames++) {
        agreeConfig.xAxis.labels.series[xAxisNames] = insertData.tree.root.children[xAxisNames].values[0].value;
    }
            ...
            if (!oParam.type) {
                ...
                if (sToolTipFunctionName) {
                    ...
                    switch (sChartType.toLowerCase()) {
                        default: oToolTip.innerHTML = '<b>' + oSeries.name + '</b><br/>' + oLabels[iSelectedIndex] + ': ' + Math.round(oSeries.data[iSelectedIndex] * 100) / 100 || 0;
                        break;
                    }
                }
            } else {
