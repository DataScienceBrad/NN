    ...
    export class Visual implements IVisual {
        ...
        public update(options: VisualUpdateOptions) {
            ...
            if (dataView.metadata.columns && dataView.metadata.columns.length === 2) {
                if (dataView.metadata.columns[0].roles["category"]) {
                    this.legendFormatter = ValueFormatter.create({ format: dataView.metadata.columns[0].format });
                }
                else {
                    this.legendFormatter = ValueFormatter.create({ format: dataView.metadata.columns[1].format });
                }
            }else{
                return;
            }
            ...
            if (!this.chartmultibarDiv || settingsChanged || ((options.type & VisualUpdateType.Resize) || options.type & VisualUpdateType.ResizeEnd)) {
                ...
                this.chartmultibarDiv = myMAQlibrary(dataView, viewModel, this.settings, ValueFormatter);
            }
        }
        /**
                ...
                case 'colorSelector':
                    for (let backGroundPoint of this.backGroundPoints) {
                        objectEnumeration.push({
                            ...
                            displayName: this.legendFormatter.format(backGroundPoint.category),
                            properties: {
                                fill: {
                                    solid: {
        ...
        private getSettings(objects: DataViewObjects): boolean {
            ...
            if (typeof this.settings == 'undefined' || (JSON.stringify(objects) !== JSON.stringify(this.prevDataViewObjects))) {
                this.settings = {
                    ...
                    xAXisFontSize: getValue<number>(objects, 'XAxis', 'xAXisFontSize', 50),
                    yAxisEnableLabels: getValue<Fill>(objects, 'YAxis', 'yAxisEnableLabels', true),
                    yAXisFontSize: getValue<number>(objects, 'YAxis', 'yAXisFontSize', 50),
                    yAxisDataLabelEnable: getValue<Fill>(objects, 'YAxis', 'yAxisDataLabelEnable', true),
                    yAXisDataLabels: getValue<number>(objects, 'YAxis', 'yAXisDataLabels', 50),
