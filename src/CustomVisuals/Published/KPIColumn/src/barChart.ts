module powerbi.extensibility.visual {
    ...
    function visualTransform(options: VisualUpdateOptions, host: IVisualHost, context: any): BarChartViewModel {
        ...
        for (let i = 0, len = Math.max(category.values.length, dataValue.values.length); i < len; i++) {
            ...
            let formatter = ValueFormatter.create({ format: options.dataViews[0].categorical.categories[0].source.format });
    ...
    export class BarChart implements IVisual {
        ...
        public update(options: VisualUpdateOptions) {
            ...
            if (options.viewport.height > 100) {
                ...
                let dataLabels: DataLabels = this.getDataLabelSettings(this.dataViews);
                ...
                if (viewModel.dataMax === 0) {
                    ...
                    let yTitleTooltip = valueFormatter.create({
                        format: options.dataViews[0].categorical.values[0].source.format
                    });
                    ...
                    if (dataLabels.show) {
                        ...
                        let targetLinesHeight = $('.xTargetAxis').attr('y1');
                        let displayValLabels = 0;
                        ...
                        let labelMargin = { top: 20, right: 10, left: 2 };
                        this.barContainer
                            .append('g')
                            ...
                            .text(function (d) {
                                let labelFormattedText = formatter.format(d.value);
                            ...
                            .attr({
                                x: d => xScale(d.category) + xScale.rangeBand() / 2 + 2,
                                y: d => yScale(<number>d.value) - 4,
                                'font-size': dataLabels.fontSize + 'px',
                                'font-weight': "bold"
                            });

                        //if labels cross height then place inside bars
                        $('.labelGraphicContext').find('text').each(function () {
                            let labelHeight = $(this).height();
                            let barlabel = $(this).attr('y');
                            let diff = parseInt(barlabel) - 0;
                            if (diff < labelHeight) {
                                $(this).attr('y', parseInt(barlabel));
                            }
                        });
                    }
        ...
        public getDataLabelSettings(dataView: DataView): DataLabels {
            let dataLabelsSettings: DataLabels = this.getDefaultDataLabelSettings();
            let objects: DataViewObjects = null;
            if (!dataView.metadata || !dataView.metadata.objects) {
                return dataLabelsSettings;
            }
            objects = dataView.metadata.objects;
            let dataProps = chartProperties.dataLabels;
        ...
        public getDefaultDataLabelSettings(): DataLabels {
            return {
                show: false,
                fontColor: '#7c7c7c',
                fontSize: 11,
        ...
        private getTooltipData(value: any): VisualTooltipDataItem[] {
            ...
            let ytdDisplayName = "";
            this.dataViews.metadata.columns.forEach(element => {
