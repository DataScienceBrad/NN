...
function renderMAQVisual(objDataView, formatOption, viewPort, valueFormatter) {
    var dIterator = 0;
    var injectData = objDataView;
    var insertData = Object.getPrototypeOf(injectData);
    var allSameModeling = 1;
    var cloudConfig = {
        "chart": {
            "renderTo": "cloudChart",
            "isResponsive": true,
        ...
        "xAxis": {
            ...
            "labels": {
                ...
                "rotation": 0,
                "style": {
                    "fill": formatOption.xAxisLabelColor,
                    "fontSize": "",
                    "fontFamily": "Segoe UI"
                },
                "x": 0,
        ...
        "yAxis": {
            ...
            "labels": {
                ...
                "rotation": 0,
                "style": {
                    "fill": formatOption.yAxisLabelColor,
                    "fontSize": "",
                    "fontFamily": "Segoe UI"
                },
                "x": 0,
    ...
    cloudConfig.xAxis.labels.style.fontSize = (formatOption.xAXisFontSize * 30) / 40;
    cloudConfig.yAxis.labels.style.fontSize = (formatOption.yAXisFontSize * 30) / 40;
    cloudConfig.xAxis.labels.style.fill = formatOption.xAxisLabelColor;
    cloudConfig.yAxis.labels.style.fill = formatOption.yAxisLabelColor;
    ...
    if (!objDataView || !objDataView.metadata || !objDataView.metadata.columns || objDataView.metadata.columns.length === 0
        ...
        d3.select("#cloudChart").attr({ "style": null });
        d3.select("#cloudChart").append("svg").attr({ "width": window.frames.innerWidth + "px", "height": window.frames.innerHeight + "px" });
    ...
    var widthMap = 0;
    var maxParent = 0;
    var parentRecord = "parent0"
    var childRecord = "child0"
    ...
    for (var i = 1; i <= 5; i++) {
        ...
        if (roles[parentRecord].length > 0) {
            ...
            parentList.push(i);
            maxParent = i;
            cloudConfig.series[parentMapper + i].name = objDataView.metadata.columns[roles[parentRecord][0]].displayName;
            cloudConfig.series[parentMapper + i].formatter = valueFormatter.create({ format: objDataView.metadata.columns[roles[parentRecord][0]].format });
            for (j = 0; j < totalRowCount; j++) {
                if (!(typeof (objDataView.table.rows[j][roles[parentRecord][0]]) === "string" || typeof (objDataView.table.rows[j][roles[parentRecord][0]]) === "Object")) {
                    cloudConfig.series[parentMapper + i].data.push(objDataView.table.rows[j][roles[parentRecord][0]]);
                }
                else {
                    cloudConfig.series[parentMapper + i].data.push("");
                    inValidDataCounter++;
                }
            }
        ...
        if (roles[childRecord].length > 0 && roles[parentRecord].length > 0) {
            inValidDataCounter = 0;
            cloudConfig.series[childMapper + i].name = objDataView.metadata.columns[roles[childRecord][0]].displayName;
            cloudConfig.series[childMapper + i].formatter = valueFormatter.create({ format: objDataView.metadata.columns[roles[childRecord][0]].format });
            for (j = 0; j < objDataView.table.rows.length; j++) {
                if (!(typeof (objDataView.table.rows[j][roles[childRecord][0]]) === "string" || typeof (objDataView.table.rows[j][roles[childRecord][0]]) === "object")) {
                    cloudConfig.series[childMapper + i].data.push(objDataView.table.rows[j][roles[childRecord][0]]);
                }
                else {
                    cloudConfig.series[childMapper + i].data.push("");
                    inValidDataCounter++;
                }
            }
            inValidColumnCounter = inValidDataCounter === totalRowCount ? inValidColumnCounter + 1 : inValidColumnCounter;
    ...
    if (parentPresentCounter === 0 && childPresentCounter !== 0) {
        ...
        d3.select("svg").append("text")
            .attr({ "x": (window.frames.innerWidth / 2) + "px", "y": (window.frames.innerHeight / 2) + "px", "font-size": "20px", "text-anchor": "middle" })
            .text("No Bars available! Bullets can't be shown");
        return;
    }
    if (inValidColumnCounter === widthMap) {
        if (inValidColumnCounter !== 0) {
            d3.select("#cloudChart").attr({ "style": null });
            d3.select("#cloudChart").append("svg").attr({ "width": window.frames.innerWidth + "px", "height": window.frames.innerHeight + "px" });
            d3.select("svg").append("text")
                .attr({ "x": (window.frames.innerWidth / 2) + "px", "y": (window.frames.innerHeight / 2) + "px", "font-size": "20px", "text-anchor": "middle" })
    ...
    while (i < 10) {
        ...
        cloudConfig.plotOptions.bar.color.pop();
        widthMap--;
    }
    for (var maxParentCounter = maxParent; maxParentCounter >= 1; maxParentCounter--) {
        if (parentList.indexOf(maxParentCounter) === -1) {
            cloudConfig.series.splice((maxParentCounter - 1) * 2, 2);
            cloudConfig.plotOptions.bar.color.splice((maxParentCounter - 1) * 2, 2);
        }
    }
    var objChildEmpty = {
        childenabled: true,
    ...
    var yAxisNames = 0;
    for (yAxisNames = 0; yAxisNames < insertData.categorical.categories[0].values.length; yAxisNames++) {
        cloudConfig.yAxis.labels.series[yAxisNames] = insertData.categorical.categories[0].values[yAxisNames] === "" ? "N/A" : insertData.categorical.categories[0].values[yAxisNames];
    }
    for (legendName = 0; legendName < cloudConfig.series.length; legendName++) {
        if (cloudConfig.series[legendName]) {
            cloudConfig.series[legendName].enabled = getLegendEnable()[legendName];
        }
    }
    var yFormatter = valueFormatter.create({ format: objDataView.metadata.columns[roles["categoryname"][0]].format });
    var xFormatter = valueFormatter.create({ format: null });
    ...
    while (parentCounter <= 5) {
        if (roles["parent" + parentCounter] && objDataView.metadata.columns[roles["parent" + parentCounter][0]] && objDataView.metadata.columns[roles["parent" + parentCounter][0]].format) {
            xFormatter = valueFormatter.create({ format: objDataView.metadata.columns[roles["parent" + parentCounter][0]].format });
            break;
        }
        parentCounter++;
    }
    ...
    (function (cloudConfig) {
        ...
        MAQ.applyStyle = function (oELE, oStyle) {
            ...
            if (oStyle && typeof oStyle === 'object') {
                ...
                for (iStyleCounter = iNumOfStyle; 0 <= iStyleCounter; iStyleCounter -= 1) {
                    if (oStyleColl[iStyleCounter] === "fontSize") {
                        oELE.style[oStyleColl[iStyleCounter]] = oStyle[oStyleColl[iStyleCounter]] + 'px';
                    }
        ...
        */
        MAQ.getNormalized_Min_Max_Interval = function (iMin, iMax, iNumberOfAxis) {
            ...
            if (iMax - iMin === 0) {
                ...
                else if (iMax > 0) //all are positive
                {
                    iMin = 0;
        ...
        function showToolTip(evt, oParam) {
            ...
            if (!oParam.type) {
                ...
                if (sToolTipFunctionName) {
                    ...
                    var sChartType = oConfig.chart.type, iLen, i;
                    oToolTip.innerHTML = '<b>' + oSeries.name + '</b><br/>' + yFormatter.format(oLabels[iSelectedIndex]) + ': ' + oSeries.formatter.format(oSeries.data[iSelectedIndex]);
        ...
        MAQ.drawLegend = function (chartConfigOptions) {
            ...
            if (oLegend.enabled) {
                ...
                if (chartConfigOptions.series.length > 0) {
                    ...
                    for (iSCounter = 0; iSCounter < iNumSymbols; iSCounter += 1) {
                        if (chartConfigOptions.series[iSCounter].name == "" || chartConfigOptions.series[iSCounter].data.length === 0) {
                            continue;
                        }
        ...
        MAQ.getAxisSpacing = function (chartConfigOptions, oAxis, sBigData, bFlag) {
            ...
            if (oAxisLabel.enabled) {
                if (!bFlag) {
                    ...
                    sBigData = xFormatter.format(sBigData);
                }
            } else {
                sBigData = '';
            }
            ...
            if (bFlag) {
                ...
                if (sBigData.length > 8) {
                    oDimAxis.width = (oDimAxis.width / sBigData.length) * 10;
                }
                else if (sBigData.length && sBigData.length < 3) {
                    oDimAxis.width = (oDimAxis.width / sBigData.length) * 3;
                }
        ...
        */
        MAQ.drawAxis = function (chartConfigOptions) {
            ...
            if (isNaN(leftSpacing)) {
                leftSpacing = 100;
            }
            leftSpacing = leftSpacing > 100 ? 100 : leftSpacing;
            ...
            for (iCounter = yAxisSeriesLength; yAxisLimit <= iCounter; iCounter -= 1) {
                if (bSkipPlot) {
                    ...
                    if (yAxisLabel.enabled) {
                        ...
                        if (oAttrLabel.text === null || oAttrLabel.text === "") {
                            oAttrLabel.text = "N/A";
                        }
            ...
            var maxWidth = 0, oneWidth = 0, maxCharacters = 0, maxAllowedCharacters = 0;
            //space for one character
            oAttrLabel.text = 's';
            oLabel = MAQ.createSVGElement(chartConfigOptions.svgNS, 'text', oAttrLabel);
            oGrpXAxisLblNTick.appendChild(oLabel);
            oDim = MAQ.getObjectDimension(oLabel);
            oGrpXAxisLblNTick.removeChild(oLabel);
            oneWidth = oDim.width;
            for (iCounter = 0; iCounter < xAxisSeries.length; iCounter += 1) {
                oAttrLabel.text = String(xAxisSeries[iCounter]);
                if (maxCharacters < oAttrLabel.text.length) {
                    maxCharacters = oAttrLabel.text.length;
                }
            }
            for (iCounter = 0; iCounter < xAxisSeries.length; iCounter += 1) {
                isDrillBar = false;
                ...
                if (bSkipPlot) {
                    ...
                    if (xAxisLabel.enabled) {
                        oGrpXAxisLblNTick.appendChild(oTick); //show the tick iff the x-axis label is enabled
                        iPrevX = oAttrLabel.x;
                        oAttrLabel.text = xFormatter.format(xAxisSeries[iCounter]);
                        /* Code for clipping the text to specified number of characters */
                        sTempText = oAttrLabel.text;
                        if (sTempText.length > iNumOfCharsAllowed && iNumOfCharsAllowed >= 2) {
                            oAttrLabel.text = oAttrLabel.text.substring(0, Math.max(iNumOfCharsAllowed - 3, 2)) + '...';
                        }
        ...
        */
        MAQ.createBarChart = function (chartConfigOptions) {
            'use strict';
            d3.select("#cloudChart").style({ "overflow-y": "hidden", "overflow-x": "hidden" });
            var iMinRerquiredHeight = chartConfigOptions.yAxis.labels.series.length * 70;

            if (iMinRerquiredHeight > chartConfigOptions.availHeight) {
                chartConfigOptions.availHeight = iMinRerquiredHeight;
                d3.select(".MAQChartsSvgRoot").style({ "height": chartConfigOptions.availHeight + 'px' });
                d3.select("#cloudChart").style("overflow-y", "scroll");
            }

            if (chartConfigOptions.availHeight < 270 && chartConfigOptions.availWidth < 630) {
                chartConfigOptions.availHeight = 270;
                chartConfigOptions.availWidth = 630;
                d3.select(".MAQChartsSvgRoot").style({ "height": chartConfigOptions.availHeight + 'px', "width": chartConfigOptions.availWidth + 'px' });
                d3.select("#cloudChart").style({ "overflow-y": "auto", "overflow-x": "auto" });
            }
            if (chartConfigOptions.availWidth < 630) {
                chartConfigOptions.availWidth = 630;
                d3.select(".MAQChartsSvgRoot").style("width", chartConfigOptions.availWidth + 'px');
                d3.select("#cloudChart").style("overflow-x", "auto");
            }
            if (chartConfigOptions.availHeight < 270) {
                chartConfigOptions.availHeight = 270;
                d3.select(".MAQChartsSvgRoot").style("height", chartConfigOptions.availHeight + 'px');
                d3.select("#cloudChart").style("overflow-y", "auto");
            }
            chartConfigOptions.plotOptions.bar.padding = (chartConfigOptions.availHeight * 3) / (7 * chartConfigOptions.yAxis.labels.series.length);
            ...
            if (oDataInfo.min || oDataInfo.max) {
                ...
                if (!oColumnPlotOptions.stacked) {
                    oBarWidth = oColumnPlotOptions.fixedWidth || oBarWidth / numberOfParents;
                }
                var oRectAttr = {
                    x: 0,
                    y: 0,
                ...
                for (iSeriesCounter = 0; iSeriesCounter < iSeriesLength; iSeriesCounter += 1) {
                    ...
                    for (iCounter = 0; iCounter < iLength; iCounter += 1) {
                        ...
                        if (isSeriesEnabled(oSeries, iSeriesCounter)) {
                            ...
                            if (oSeries[iSeriesCounter].data[iCounter] === "" || oSeries[iSeriesCounter].data[iCounter] === 0 || oSeries[iSeriesCounter].data[iCounter] === null) {
