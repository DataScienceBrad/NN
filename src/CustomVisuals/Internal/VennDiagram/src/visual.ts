module powerbi.extensibility.visual {
    ...
    function visualTransform(options: VisualUpdateOptions, host: IVisualHost, context: any): VennViewModel {
        ...
        for (let iIterator = 0; iIterator < numberOfObjects; iIterator++) {
            let defaultColors = [{ solid: { color: "#01B8AA" } }, { solid: { color: "#374649" } }, { solid: { color: "#FD625E" } }, { solid: { color: "#F2C80F" } }];
            VennDataPoints.push({
                category: context.finalSingleObjects[iIterator],
                color: getCategoricalObjectValue<Fill>(categorical.categories[iIterator], iIterator, "colors", "colorToFill", defaultColors[iIterator]).solid.color,
                selectionId: host.createSelectionIdBuilder()
                    .withCategory(categorical.categories[iIterator], iIterator)
                    .createSelectionId(),
                value: context.finalUpdatedSingleObjectsValues[iIterator],
            });
        }
        return {
    ...
    export class VennDiagram implements IVisual {
            ...
            for (let iCounter = 0; iCounter < VennDataPoint.length; iCounter++) {
                if (dataView && dataView.categorical && dataView.categorical.categories && dataView.categorical.categories[0]) {
                    legendData.dataPoints.push({
                        ...
                        identity: host.createSelectionIdBuilder().withCategory(dataView.categorical.categories[iCounter], iCounter).createSelectionId(),
                        label: VennDataPoint[iCounter].category,
                        measure: sumObj[VennDataPoint[iCounter].category] ? sumObj[VennDataPoint[iCounter].category] : 0, // this.singleTempValue[iCounter],
                        selected: false,
                    });
                    legendValues[iCounter] = VennDataPoint[iCounter].category;
                }
                // measureSum += sumArr[iCounter]; // this.singleTempValue[iCounter];
            }
            legendData.measureSum = totalSum;
            // legendData.measureSum = measureSum;
            return legendData;
        }
        ...
        public update(options: VisualUpdateOptions) {
            ...
            $(".ErrorMessage").remove();
            // this.rootElement.select(".legend #legendGroup").selectAll("*").remove();
            if (isInvalidData) {
        ...
        public adjustVisual(legendSetting: LegendSetting) {
            ...
            if (legendSetting.show) {
                ...
                switch (legendOrient) {
                    ...
                    case 2:
                    case 7:
                        // this.svg.style("margin-right", legendWidth + "px");
        ...
        public Join(text: string) {
            let len = text.length;
            for (let iCounter = 0; iCounter < len; iCounter++) {
                if (text[iCounter] === "$") {
                    return true;
                }
            }
        ...
        public drawOneObjects(width: number, height: number, marginX: number, marginY: number, color: any) {
            ...
            this.tooltipServiceWrapper.addTooltip(this.svg.selectAll("circle, text"),
        ...
        public drawFourObjects(width: number, height: number, marginX: number, marginY: number, color: any) {
            ...
            let mainGroup = this.mainGroup;
            let finalUpdatedOtherObjectsValues = this.finalUpdatedOtherObjectsValues;
            width = width - 2 * marginX - this.min(width, height) / 32;
            height = height - 2 * marginY;
            let circleRadius = this.min(width, height) / 3;
        ...
        public enumerateObjectInstances(options: EnumerateVisualObjectInstancesOptions): VisualObjectInstanceEnumeration {
            ...
            switch (objectName) {
                case "legend":
                    ...
                    break;
                case "colors":
                    for (let vennPoint of this.vennPoints) {
                        objectEnumeration.push({
                            displayName: vennPoint.category,
                            ...
                            selector: vennPoint.selectionId.getSelector(),
                        });
                    }
                    break;
                case "labelSettings":
