module powerbi.extensibility.visual {
    ...
    export class DonutChart implements IVisual {
        ...
        public update(options: VisualUpdateOptions) {
            ...
            let svg = d3.select('.ring_donutChart').append('svg')
                .attr('width', donutWidth)
                .attr('height', donutHeight)
                .append('g')
                .attr('transform', `translate(${donutWidth / 2},${donutHeight / 2})`);

            let g = svg.selectAll('.ring_arc')
            ...
            if (lablesettings.show) {
                let enteringLabels = svg.selectAll('.ring_polyline').data(pie(viewModel.dataPoints)).enter();
                ...
                let dataLabels = this.svg.selectAll('.ring_donutChart g.ring_labelName text');
                ...
                for (let i = 0; i < dataLabelArrLen; i++) {
                    if (detaillabelprop.labelStyle !== 'Data' && detaillabelprop.labelStyle !== 'Category'
                        && detaillabelprop.labelStyle !== 'Percent of total') {
                        let enteringSecondRowtext = svg.selectAll('.ring_secondaryLabelName').data(pie(viewModel.dataPoints)).enter();
                        let secondarytextGroups = enteringSecondRowtext.append('g').attr('class', 'ring_secondaryLabelName');
            ...
            if (lablesettings.show) {
                ...
                for (let i = 0; i < labelsLength; i++) {
                    let obj1 = document.getElementById(`ring_label_${i}`).getBoundingClientRect();
                    for (let j = i + 1; j <= labelsLength - 1; j++) {
                        let obj2 = document.getElementById(`ring_label_${j}`).getBoundingClientRect();
                        let obj3;
                    ...
                    labelYPos = $(`#ring_label_${i}`).attr('y');
            ...
            let arcs = this.svg.selectAll('.ring_arc').data(viewModel.dataPoints);
            ...
            arcs.on('click', function (d) {
                selectionManager.select(d.selectionId).then((ids: any[]) => {
                    ...
                    let legend: any = THIS.rootElement.selectAll('.ring_legend .legendItem');
            ...
            $('.ring_legend #legendGroup').on('click.load', '.navArrow', function () {
                THIS.addLegendSelection();
            });
            ...
            if (animationSettings.show) {
                let donutArcPath = this.svg.selectAll('.ring_arc path');
                ...
                if (viewModel.primaryMeasureSum) {
                    ...
                    if (viewModel && viewModel.secondaryMeasureSum) {
                        ...
                        if (smIndicator.show) {
                            ...
                            let element = this.rootElement.select('.ring_SecondaryValue');
                    ...
                    let pContainerDivWidth = parseFloat(this.rootElement.select('.ring_pContainer').style('width'));
                    ...
                    let pContainerDivHeight = parseFloat(this.rootElement.select('.ring_pContainer').style('height'));
                    let summarizedDivHeight = parseFloat(this.rootElement.select('.ring_SummarizedDivContainer').style('height'));
        ...
        private addLegendSelection() {
            ...
            let legends = this.rootElement.selectAll('.ring_legend .legendItem');
            let selectionManager = this.selectionManager;
