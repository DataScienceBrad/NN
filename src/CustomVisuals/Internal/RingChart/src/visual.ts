module powerbi.extensibility.visual {
    ...
    export interface secondarySummaryLabels {
        color: string;
        labelDisplayUnits: number;
        labelPrecision: number;
        fontSize: number;
    ...
    export let chartProperties = {
        ...
        secondarySummaryLabels:
        {
            ...
            labelDisplayUnits: <DataViewObjectPropertyIdentifier>{ objectName: "secondarySummaryLabels", propertyName: "labelDisplayUnits" },
    ...
    function visualTransform(options: VisualUpdateOptions, host: IVisualHost, context: any): DonutChartViewModel {
        ...
        let primarykpiSum = 0, secondarykpiSum = 0;
        ...
        let isLegendAvailable = false, isPrimaryMeasureAvailable = false;
        ...
        for (let i = 0, len = Math.max(category.values.length, dataValue.values.length); i < len; i++) {
            ...
            let donutDataPoint: DonutChartDataPoint = {
                category: "",
                color: "",
                primaryKPIValue: "",
                primaryName: "",
            ...
            for (let cat1 = 0; cat1 < dataViews[0].categorical.categories.length; cat1++) {
                let dataView: DataView = dataViews[0];
                if (dataView.categorical.categories[cat1].source.roles["Category"]) {
                    donutDataPoint.category = dataView.categorical.categories[cat1].values[i] ? (dataView.categorical.categories[cat1].values[i].toString()) : "";
                }
                let tooltipDataPoint: TooltipDataPoints = {
                    formatter: "",
                    name: dataView.categorical.categories[cat1].source.displayName,
                    value: dataView.categorical.categories[cat1].values[i] ? (dataView.categorical.categories[cat1].values[i].toString()) : "",
                };
            ...
            for (let k = 0; k < dataViews[0].categorical.values.length; k++) {
                ...
                if (dataView.categorical.values[k].source.roles["Y"]) {
                    ...
                    primaryMeasureSum += parseFloat(dataView.categorical.values[k].values[i] ? dataView.categorical.values[k].values[i].toString() : "0");
                }
                if (dataView.categorical.values[k].source.roles["SecondaryMeasure"]) {
                    ...
                    secondaryMeasureSum += parseFloat(dataView.categorical.values[k].values[i] ? dataView.categorical.values[k].values[i].toString() : "0");
                }
                if (dataView.categorical.values[k].source.roles["PrimaryKPI"]) {
                    ...
                    primarykpiSum += parseFloat(dataView.categorical.values[k].values[i] ? dataView.categorical.values[k].values[i].toString() : "0");
                }
                if (dataView.categorical.values[k].source.roles["SecondaryKPI"]) {
                    ...
                    secondarykpiSum += parseFloat(dataView.categorical.values[k].values[i] ? dataView.categorical.values[k].values[i].toString() : "0");
                }
                let tooltipDataPoint: TooltipDataPoints = {
    ...
    export class DonutChart implements IVisual {
        ...
        public update(options: VisualUpdateOptions) {
            ...
            let noDataMessage: nodataText = this.getNoDataText(this.dataViews);
            let errorMsg = noDataMessage.textMessage;

            if (!viewModel || !viewModel.isLegendAvailable || !viewModel.isPrimaryMeasureAvailable) {
                let errorHtmlChunk = "<div class='ErrorMessage' title='Please select Primary Measure and Legend values'>Please select 'Primary Measure' and 'Legend' values</div>";
                $("#sandbox-host").append(errorHtmlChunk);
                return;
            } else if (!viewModel.legendData || !viewModel.primaryMeasureSum) {
                let errorHtmlChunk = "<div class='ErrorMessage' title='" + errorMsg + "'>" + errorMsg + "</div>";
                $("#sandbox-host").append(errorHtmlChunk);
                return;
            }
            ...
            let donutTitleSettings: donutTitle = this.getDonutTitle(this.dataViews);
            ...
            if (donutTitleSettings.show) {
                this.rootElement.select(".donutTitle").style({ "display": "block", "position": "absolute" });
                let textProperties: TextProperties = {
                    fontFamily: This.defaultFontFamily,
                    fontSize: PixelConverter.fromPoint(donutTitleSettings.fontSize), // + "px",
                ...
                this.rootElement.select(".donutTitle")
                    ...
                    .style({
                        ...
                        "font-size": PixelConverter.fromPoint(donutTitleSettings.fontSize), // + "px",
                    })
                    .text(finalText)
                    .append("span")
                    .text(" (?)")
            ...
            if (legendSettings.show) {
                this.rootElement.select(".legend").style({ "top": donutTitleHeight + "px" });
                ...
                let label = textGroups
                    ...
                    .text(function (d) {
                        ...
                        if (This.dataViews && This.dataViews.categorical && This.dataViews.categorical.values && This.dataViews.categorical.values[0]) {
                            primaryFormatter = This.dataViews.categorical.values[0].source.format ? This.dataViews.categorical.values[0].source.format : "";
                        }
                        ...
                        if (detaillabelprop.labelStyle === "Data") {
                            ...
                            text = (d.data.value / summaryvalue * 100).toFixed(detaillabelprop.labelPrecision).toString() + "%";
                        ...
                        if (finalText.indexOf("...") > -1
                            ...
                            if (position === 1) {
                                ...
                                let textEnd = pos[0] + (-1 * widthOfText);
                                if (textEnd < (-1 * donutWidth / 2)) {
                                    finalText = textMeasurementService.getTailoredTextOrDefault(textProperties, pos[0] + donutWidth / 2);
                                    if (finalText.length < 4) {
                                        return "";
                    ...
                    .text(function (d) {
                        ...
                        let summaryvalue = viewModel.primaryMeasureSum, text;
                        if (detaillabelprop.labelStyle === "Data") {
                            text = formatter.format((d.data.value));
                        } else if (detaillabelprop.labelStyle === "Category") {
                            text = d.data.category;
                    ...
                    if (detaillabelprop.labelStyle !== "Data" && detaillabelprop.labelStyle !== "Category"
                        ...
                        let secondRowLabel = secondarytextGroups
                            ...
                            .text(function (d) {
                                ...
                                let text = "";
                                let summaryvalue = viewModel.primaryMeasureSum;
                                if (detaillabelprop.labelStyle === "Category, percent of total" || detaillabelprop.labelStyle === "Data value, percent of total") {
                                    text = "(" + (d.data.value / summaryvalue * 100).toFixed(2).toString() + "%" + ")";
                                } else if (detaillabelprop.labelStyle === "Both") {
                                    text = "(" + formatter.format(d.data.value) + ")";
                                } else {
                        ...
                        upperLabelText = dataLabelsArr[i] && dataLabelsArr[i].childNodes && dataLabelsArr[i].childNodes[0] && dataLabelsArr[i].childNodes[0].textContent ?
                            dataLabelsArr[i] && dataLabelsArr[i].childNodes && dataLabelsArr[i].childNodes[0] && dataLabelsArr[i].childNodes[0].textContent : "no data";
                        let expString = "";
                        if (detaillabelprop.labelStyle === "Category, percent of total"
            ...
            if (lablesettings.show) {
                ...
                for (let i = 0; i < labelsLength; i++) {
                    ...
                    for (let j = i + 1; j <= labelsLength - 1; j++) {
                        ...
                        let obj3;
                        let cond_expr = !(obj2.left > obj1.right ||
                        ...
                        if (detaillabelprop.labelStyle !== "Data"
                            ...
                            cond_expr = !(obj2.left > obj1.right ||
                                obj2.right < obj1.left ||
                                obj2.top > obj1.bottom ||
                                obj2.bottom < obj1.top)
                                ||
                    ...
                    let labelYPos: any = 0, secondLabelYPos: any = 0;
                    ...
                    secondLabelYPos = Math.abs(parseFloat($("#secondRowLabel_" + i).attr("y"))) ?
                        Math.abs(parseFloat($("#secondRowLabel_" + i).attr("y"))) + 3 : 0; // 0.2em is the dy value. On conversion to px it will be 3px
                    let visualHeight: any = donutHeight / 2;
            ...
            let animationSettings: animation = this.getAnimation(this.dataViews);
        ...
        public drawSummaryDiv(radius, options, viewModel, legendHeight, legendWidth, summaryLabelSettings, dataViews) {
            if (summaryLabelSettings.show) {
                if (viewModel.primaryMeasureSum) {
                    ...
                    let donutTitleSettings: donutTitle = this.getDonutTitle(this.dataViews);
                    let secondarySummarySettings: secondarySummaryLabels = this.getSecondarySummaryLabels(this.dataViews);
                    ...
                    let primaryFormatter = ValueFormatter.create({
                        format: options.dataViews[0].categorical.values[0].source.format ? options.dataViews[0].categorical.values[0].source.format : "",
                    ...
                    if (!this.legendObjectProperties) {
                        ...
                        y = halfViewPortHeight - (innerRadius / Math.SQRT2) + parseInt(legendHeight.toString(), 10) / 2 + parseInt(donutTitleHeight.toString(), 10) / 2;
                    ...
                    if (this.currentViewport.width > 150 && this.currentViewport.height > 100) {
                        ...
                        if (pmIndicator.show) {
                            ...
                            if (viewModel.primaryKPISum) {
                                threshold_Value = viewModel.primaryKPISum;
                            ...
                            if (threshold_Value <= viewModel.primaryMeasureSum) {
                                this.rootElement.select(".TotalValue")
                                    .html("<div class='primaryMeasureSum' title = '" + primaryTooltipFormatter.format(viewModel.primaryMeasureSum) + "'" + ">" + primaryFormatter.format(viewModel.primaryMeasureSum) +
                                    "</div><span class='primaryMeasureIndicator' style='color:" + upColor + "'>&#9650;</span>");
                            } else {
                                this.rootElement.select(".TotalValue")
                                    .html("<div class='primaryMeasureSum' title = '" + primaryTooltipFormatter.format(viewModel.primaryMeasureSum) + "'" + ">" + primaryFormatter.format(viewModel.primaryMeasureSum) +
                                    "</div><span class='primaryMeasureIndicator' style='color:" + downColor + "'>&#9660;</span>");
                            }

                        } else {
                            this.rootElement.select(".TotalValue")
                                .html("<div class='primaryMeasureSum' title = '" + primaryTooltipFormatter.format(viewModel.primaryMeasureSum) + "'" + ">" + primaryFormatter.format(viewModel.primaryMeasureSum) + "</div>");
                        }
                    }
                    ...
                    if (viewModel && viewModel.secondaryMeasureSum) {
                        ...
                        secondaryFormatter = ValueFormatter.create({
                            format: options.dataViews[0].categorical.values[1].source.format ? options.dataViews[0].categorical.values[1].source.format : "",
                            precision: secondarySummarySettings.labelPrecision,
                            value: secondarySummarySettings.labelDisplayUnits === 0 ? secondaryFormatterVal : secondarySummarySettings.labelDisplayUnits,
                        });
                        ...
                        if (dataViews && dataViews.categorical && dataViews.categorical.values
                            && dataViews.categorical.values[1] && dataViews.categorical.values[1].source && dataViews.categorical.values[1].source.format
                        ...
                        this.rootElement.select(".pContainer")
                            ...
                            .style({
                                ...
                                "font-size": PixelConverter.fromPoint(secondaryFontSize), // + "px",
                                "margin": "0",
                                "overflow": "hidden",
                                "text-align": "center",
                        ...
                        if (smIndicator.show) {
                            ...
                            if (smThreshold <= viewModel.secondaryMeasureSum) {
                                this.rootElement.select(".SecondaryValue")
                                    .html("<div class='secondaryMeasureSum' title = '" + secondaryTooltipFormatter.format(viewModel.secondaryMeasureSum) + "'" + ">" + secondaryFormatter.format(viewModel.secondaryMeasureSum) +
                                    "</div><span class='secondaryMeasureIndicator' style='color:" + upColor + "'>&#9650;</span>");
                            } else {
                                this.rootElement.select(".SecondaryValue")
                                    .html("<div class='secondaryMeasureSum' title = '" + secondaryTooltipFormatter.format(viewModel.secondaryMeasureSum) + "'" + ">" + secondaryFormatter.format(viewModel.secondaryMeasureSum) +
                                    "</div><span class='secondaryMeasureIndicator' style='color:" + downColor + "'>&#9660;</span>");
                            }
                        } else {
                            this.rootElement.select(".SecondaryValue")
                                .html("<div class='secondaryMeasureSum' title = '" + secondaryTooltipFormatter.format(viewModel.secondaryMeasureSum) + "'" + ">" + secondaryFormatter.format(viewModel.secondaryMeasureSum) + "</div>");
                        }
                    }
                    ...
                    let formattedPrimaryMeasureSumTextProperties: TextProperties = {
                        fontFamily: "Segoe UI",
                        fontSize: PixelConverter.fromPoint(summaryLabelSettings.fontSize), // + "px",
                        text: primaryFormatter.format(viewModel.primaryMeasureSum),
                    };
                    let formattedPrimaryMeasureSumTextPropertiesWidth = textMeasurementService.measureSvgTextWidth(formattedPrimaryMeasureSumTextProperties);
                    ...
                    if (this.rootElement.select(".primaryMeasureIndicator")[0][0] !== null) {
                        if (formattedPrimaryMeasureSumTextPropertiesWidth + parseFloat(this.rootElement.select(".primaryMeasureIndicator").style("width")) * 2 > pContainerDivWidth) {
                            $(".primaryMeasureSum").css("width", availableWidth);
                        }
                    }
                    if (this.rootElement.select(".secondaryMeasureIndicator")[0][0] !== null) {
                        if (formattedSecondaryMeasureSumTextPropertiesWidth + parseFloat(this.rootElement.select(".secondaryMeasureIndicator").style("width")) * 2 > pContainerDivWidth) {
                            $(".secondaryMeasureSum").css("width", availableWidth);
                        }
                    }
        ...
        public enumerateObjectInstances(options: EnumerateVisualObjectInstancesOptions): VisualObjectInstanceEnumeration {
            ...
            let secondarySummaryLabels: secondarySummaryLabels = this.getSecondarySummaryLabels(this.dataViews);
            ...
            let donutTitle: donutTitle = this.getDonutTitle(this.dataViews);
            let animationProps: animation = this.getAnimation(this.dataViews);
            let noDataProps: nodataText = this.getNoDataText(this.dataViews);
        ...
        private addLegendSelection() {
            ...
            legends.on("click", function (d) {
                selectionManager.select(d.identity).then((ids: any[]) => {
                    ...
                    let arcs: any = This.rootElement.selectAll(".arc");
            ...
            for (let j = 0; j < legendData.dataPoints.length; j++) {
                ...
                if (legendData.secondaryTitle) {
                    ...
                    legendDataTorender.dataPoints.push({
                        ...
                        measure: primaryData,
                        primaryIndicator: this.isPrimaryIndicator(legendData.dataPoints[j].measure, dataViews, viewModel.dataPoints[j].primaryKPIValue),
                        primaryTooltip: measureTooltip,
                        secondaryIndicator: this.isSecondaryIndicator(legendData.dataPoints[j].secondaryMeasure, dataViews, viewModel.dataPoints[j].secondaryKPIValue),
        ...
        private isPrimaryIndicator(indicatorValue, dataview, primaryKPI) {
            let isPrimaryIndicator, isPercentage;
            let primaryIndicator: primaryIndicator = this.getPrimaryIndicator(dataview);
            ...
            if (primaryIndicator.show) {
                let threshold_Value = 0;
                if (primaryKPI) {
                    threshold_Value = parseFloat(primaryKPI);
                    if (isPercentage) {
                        threshold_Value = threshold_Value / 100;
                    }
                } else {
        ...
        private isSecondaryIndicator(secondaryIndicator, dataview, secondaryKPI) {
            let isSecondaryIndicator: any, isPercentage: any;
            let smIndicator: secondaryIndicator = this.getSecondaryIndicator(dataview);
            for (let i = 0; i < dataview.categorical.values.length; i++) {
        ...
        private getLegendSettings(dataView: DataView): legendConfig {
            ...
            let legendSetting: legendConfig = this.getDefaultLegendSettings(dataView);
        ...
        private getDetailLable(dataView: DataView): DetailLables {
            let objects: DataViewObjects = null;
            let labelSettings: DetailLables = this.getDefaultDetailLable();
            if (!dataView.metadata || !dataView.metadata.objects) {
        ...
        private getSummaryLabels(dataView: DataView): summaryLabels {
            let objects: DataViewObjects = null;
            let summaryLabelSettings: summaryLabels = this.getDefaultSummaryLabel();
            if (!dataView.metadata || !dataView.metadata.objects) {
        ...
        private getDefaultSecondarySummaryLabel(): secondarySummaryLabels {
            return {
                color: "#777777",
                fontSize: 12,
                labelDisplayUnits: 0,
        ...
        private getSecondarySummaryLabels(dataView: DataView): secondarySummaryLabels {
            let objects: DataViewObjects = null;
            let secondarySummaryLabelSettings: secondarySummaryLabels = this.getDefaultSecondarySummaryLabel();
            if (!dataView.metadata || !dataView.metadata.objects) {
                return this.getDefaultSecondarySummaryLabel();
            }
            objects = dataView.metadata.objects;
            let detaillabelprop = chartProperties.secondarySummaryLabels;
            secondarySummaryLabelSettings.color = DataViewObjects.getFillColor(objects, detaillabelprop.color, secondarySummaryLabelSettings.color);
            secondarySummaryLabelSettings.labelDisplayUnits = DataViewObjects.getValue(objects, detaillabelprop.labelDisplayUnits, secondarySummaryLabelSettings.labelDisplayUnits);
            secondarySummaryLabelSettings.labelPrecision = DataViewObjects.getValue(objects, detaillabelprop.labelPrecision, secondarySummaryLabelSettings.labelPrecision);
            secondarySummaryLabelSettings.labelPrecision = secondarySummaryLabelSettings.labelPrecision < 0 ? 0 : (secondarySummaryLabelSettings.labelPrecision) > 8 ? 8 : (secondarySummaryLabelSettings.labelPrecision);
            secondarySummaryLabelSettings.fontSize = DataViewObjects.getValue(objects, detaillabelprop.fontSize, secondarySummaryLabelSettings.fontSize);
        ...
        private getPrimaryIndicator(dataView: DataView): primaryIndicator {
            let objects: DataViewObjects = null;
            let primaryIndicatorSettings: primaryIndicator = this.getDefaultPrimaryIndicator();
            if (!dataView.metadata || !dataView.metadata.objects) {
            ...
            primaryIndicatorSettings.upArrow = DataViewObjects.getFillColor(objects, chartIndicatorProp.upArrow, primaryIndicatorSettings.upArrow);
            primaryIndicatorSettings.downArrow = DataViewObjects.getFillColor(objects, chartIndicatorProp.downArrow, primaryIndicatorSettings.downArrow);
        ...
        private getSecondaryIndicator(dataView: DataView): secondaryIndicator {
            let objects: DataViewObjects = null;
            let secIndicatorSettings: secondaryIndicator = this.getDefaultSecondaryIndicator();
            if (!dataView.metadata || !dataView.metadata.objects) {
            ...
            secIndicatorSettings.downArrow = DataViewObjects.getFillColor(objects, chartIndicatorProp.downArrow, secIndicatorSettings.downArrow);
            return secIndicatorSettings;
        }

        private getDefaultDonutTitle(dataView: DataView): donutTitle {
            let titleText = "";
            if (dataView && dataView.categorical) {
                ...
                if (dataView.categorical.categories && dataView.categorical.categories[0] && dataView.categorical.categories[0].values) {
                    titleText += " by " + dataView.categorical.categories[0].source.displayName;
                }
            }
            return {
                backgroundColor: "#fff",
        ...
        private getDonutTitle(dataView: DataView): donutTitle {
            let objects: DataViewObjects = null;
            let donutTitleSettings: donutTitle = this.getDefaultDonutTitle(dataView);
            if (!dataView.metadata || !dataView.metadata.objects) {
            ...
            donutTitleSettings.backgroundColor = DataViewObjects.getFillColor(objects, chartIndicatorProp.backgroundColor, donutTitleSettings.backgroundColor);
            donutTitleSettings.tooltipText = DataViewObjects.getValue(objects, chartIndicatorProp.tooltipText, donutTitleSettings.tooltipText);
            return donutTitleSettings;
        }

        private getDefaultAnimation(): animation {
            return {
                show: false,
            };
        }

        private getAnimation(dataView: DataView): animation {
            let objects: DataViewObjects = null;
            let animationSettings: animation = this.getDefaultAnimation();
            if (!dataView.metadata || !dataView.metadata.objects) {
        ...
        private getDefaultNoDataText(): nodataText {
            return {
                textMessage: "",
            };
        }

        private getNoDataText(dataView: DataView): nodataText {
            let objects: DataViewObjects = null;
            let textMessage: nodataText = this.getDefaultNoDataText();
            if (!dataView.metadata || !dataView.metadata.objects) {
                return textMessage;
            }
            objects = dataView.metadata.objects;
