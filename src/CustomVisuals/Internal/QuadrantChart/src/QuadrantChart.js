...
function MAQDrawChart(DataStyle, settings, nodeData, series, assignData, valueFormatter, maxRadius, minRadius) {
    //data binding
    ...
    (function (config) {
        ...
        */
        function redrawChart(evt, oParam, isEvent) {
            ...
            config.xAxis.gridLineX = undefined;
            config.yAxis.gridLineY = undefined;
        ...
        */
        MAQ.drawAxis = function (chartConfigOptions) {
            ...
            if (settings.quadrantDivisionY === undefined || settings.quadrantDivisionY > yAxisSeries[iLength] || settings.quadrantDivisionY < yAxisSeries[0]) {
                var dist = (yAxisSeries[yAxisSeries.length - 1] - yAxisSeries[0]) / (yAxisSeries.length - 1)
                config.yAxis.gridLineY = yAxisSeries[0] + 4 * dist;
            ...
            var finalY;
            if (settings.showQuadrants) {
                if (config.yAxis.gridLineY === undefined || config.yAxis.gridLineY < yAxisSeries[0] || config.yAxis.gridLineY > yAxisSeries[(yAxisSeries.length - 1)]) {
                    var dist = (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0]) / (yAxisSeries.length - 1);
                    finalY = 4 * (chartConfigOptions.availHeight * (yAxisSeries[1] - yAxisSeries[0]) / (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0])); //as middle is always at 4th place
                    config.yAxis.gridLineY = yAxisSeries[0] + 4 * dist;
                    settings.quadrantDivisionY = config.yAxis.gridLineY;
                }
                else {
                    var dist = (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0]) / yAxisSeriesLength;
                    var distance;
                    finalY = chartConfigOptions.availHeight * (yAxisSeries[1] - yAxisSeries[0]) / (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0]);
                    if (config.yAxis.gridLineY < 0) {
                        distance = chartConfigOptions.availHeight - (((config.yAxis.gridLineY - yAxisSeries[0]) / dist) * finalY);
                        finalY = distance;
                    }
                    else {
                        distance = (((yAxisSeries[yAxisSeriesLength] - config.yAxis.gridLineY) / dist) * finalY);
                        finalY = distance;
                    }
                    settings.quadrantDivisionY = config.yAxis.gridLineY;
                }
            ...
            var finalX;
            if (settings.showQuadrants) {
                if (config.xAxis.gridLineX === undefined || config.xAxis.gridLineX < xAxisSeries[0] || config.xAxis.gridLineX > xAxisSeries[(xAxisSeries.length - 1)]) {
                    var dist = (xAxisSeries[xAxisSeriesLength] - xAxisSeries[0]) / xAxisSeriesLength;
                    finalX = 4 * (chartConfigOptions.availWidth * (xAxisSeries[1] - xAxisSeries[0]) / (xAxisSeries[xAxisSeriesLength] - xAxisSeries[0])); //as middle is always at 4th place
                    config.xAxis.gridLineX = xAxisSeries[0] + 4 * dist;
                    settings.quadrantDivisionX = config.xAxis.gridLineX;
                }
                else {
                    var dist = (xAxisSeries[xAxisSeriesLength] - xAxisSeries[0]) / xAxisSeriesLength;
                    var distance;
                    finalX = chartConfigOptions.availWidth * (xAxisSeries[1] - xAxisSeries[0]) / (xAxisSeries[xAxisSeriesLength] - xAxisSeries[0]);
                    if (config.xAxis.gridLineX < 0) {
                        distance = (((config.xAxis.gridLineX - xAxisSeries[0]) / dist) * finalX);
                        finalX = distance;
                    }
                    else {
                        distance = chartConfigOptions.availWidth - (((xAxisSeries[xAxisSeriesLength] - config.xAxis.gridLineX) / dist) * finalX);
                        finalX = distance;
                    }
                }
                oAttrGridLine = {
            ...
            //QuadrantLabels
            yAxisSeriesLength = yAxisSeries.length - 1;
            if (config.yAxis.gridLineY === undefined || config.yAxis.gridLineY < yAxisSeries[0] || config.yAxis.gridLineY > yAxisSeries[yAxisSeriesLength]) {
                var dist = (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0]) / yAxisSeriesLength;
                var distance = 4 * (chartConfigOptions.availHeight * (yAxisSeries[1] - yAxisSeries[0]) / (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0])); //as middle is always at 4th place
                config.yAxis.gridLineY = yAxisSeries[0] + 4 * dist;
                settings.quadrantDivisionY = config.yAxis.gridLineY;
            ...
            if (config.xAxis.gridLineX === undefined || typeof config.xAxis.gridLineX === 'undefined') {
                var dist = (xAxisSeries[xAxisSeriesLength] - xAxisSeries[0]) / xAxisSeriesLength;
                config.xAxis.gridLineX = xAxisSeries[0] + 4 * dist;
                settings.quadrantDivisionX = config.xAxis.gridLineX;
            }

            //calculate coordinates to show quadrant labels
            var SpaceOnLeft = 10; //to avoid overlapping from grid line or y-axis
            var gridX = SpaceOnLeft + ((chartConfigOptions.availWidth) * ((config.xAxis.gridLineX - xAxisSeries[0]) / (xAxisSeries[xAxisSeries.length - 1] - xAxisSeries[0])) / 2);
            var gridY1 = (chartConfigOptions.availHeight) * (((config.yAxis.gridLineY - yAxisSeries[0]) / 2) + (yAxisSeries[yAxisSeriesLength] - config.yAxis.gridLineY)) / (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0]);

            var gridX1 = SpaceOnLeft + (((chartConfigOptions.availWidth) * ((xAxisSeries[xAxisSeries.length - 1] - config.xAxis.gridLineX) / 2 + (config.xAxis.gridLineX - xAxisSeries[0])) / (xAxisSeries[xAxisSeries.length - 1] - xAxisSeries[0])));
            var gridY = (chartConfigOptions.availHeight) * ((yAxisSeries[yAxisSeriesLength] - config.yAxis.gridLineY) / 2) / (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0]);
            ...
            while (i < 4) {
                ...
                if (i === 0 || i === 3) {
                    availableWidth = (((chartConfigOptions.availWidth * (config.xAxis.gridLineX - xAxisSeries[0])) / (xAxisSeries[xAxisSeries.length - 1] - xAxisSeries[0])) / 2);
                }
                else {
                    availableWidth = ((chartConfigOptions.availWidth * (xAxisSeries[xAxisSeries.length - 1] - config.xAxis.gridLineX)) / ((xAxisSeries[xAxisSeries.length - 1] - xAxisSeries[0])) / 2);
                }
                if (i === 2 || i === 3) {
                    availableHeight = ((chartConfigOptions.availHeight) * ((yAxisSeries[yAxisSeriesLength] - config.yAxis.gridLineY)) / (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0])) / 2;
                }
                else {
                    availableHeight = ((chartConfigOptions.availHeight) * (config.yAxis.gridLineY - (yAxisSeries[0])) / (yAxisSeries[yAxisSeriesLength] - yAxisSeries[0])) / 2;
                }

                //Don't show a quadrant label if available vertical space is less or no space horizontally
                if (availableHeight < 11 || availableWidth === 0) {
                    i++;
                    continue;
                }
                ...
                //don't show a quadrant label when available horizontal space is less
                if (oAttrQuadrantLabel.text === "...") {
                    i++;
                    continue;
                }

                //set position for each quadrant label
                if (i === 0) {
                    oAttrQuadrantLabel.x = gridX;
                    oAttrQuadrantLabel.y = gridY1;
                }
                else if (i === 1) {
                    oAttrQuadrantLabel.x = gridX1;
                    oAttrQuadrantLabel.y = gridY1;
                }
                else if (i === 2) {

                    oAttrQuadrantLabel.x = gridX1;
                    oAttrQuadrantLabel.y = gridY;
                }
                else {
                    oAttrQuadrantLabel.x = gridX;
                    oAttrQuadrantLabel.y = gridY;
                ...
                for (iSeriesCounter = 0; iSeriesCounter < iSeriesLength; iSeriesCounter += 1) {
                    ...
                    for (iCounter = 0; iCounter < iLength; iCounter += 1) {
                        if (isSeriesEnabled(oSeries, iSeriesCounter)) {
                            ...
                            if (oDataArray.radius[iCounter] === null || oDataArray.scaleY[iCounter] === null || oDataArray.scaleX[iCounter] === null) {
                                continue;
                            }
                            ...
                            if (maxRadius - minRadius !== 0) //when all are not same
                            {
                                oBubbleAttr.r = ((oDataArray.radius[iCounter] + 1 - minRadius) / (maxRadius - minRadius)) * 12; //normalize bubble radius according to input
                            }
                            else {
                                //when all are same
                                oBubbleAttr.r = 12;
