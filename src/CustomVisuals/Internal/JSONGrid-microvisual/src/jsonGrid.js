...
function myMAQlibrary(dWagon, gridFormatters, valueFormatter) {
    ...
    var config = {
        ...
        "isWin8App": false,
        "hasGauge": false,
        "sparklineColor": gridFormatters.sparklineColor,
        "barchartColor": gridFormatters.barchartColor,
        "callBackFunc": null
    };

    var obj, sparkObj, barObj, data = [], tempSparkData = [], sparkData = [], sparkColName = [], tempBarData = [], barData = [], barColName = [];
    var gaugeObj, gaugeData = [], gaugeColName = [], tempGaugeData = [];
    ...
    for (jCount = 0; jCount < dWagon.table.rows.length; jCount++) {
        obj = {}; sparkColName = []; sparkObj = {}; barObj = {}; barColName = []; gaugeObj = {}; gaugeColName = [];
        for (iCount = 0; iCount < dWagon.table.columns.length; iCount++) {

            if (dWagon.table.columns[iCount].roles['SparkLine']) {
                ...
                if (value === null) {
                    ...
                    sparkObj[name] = dWagon.table.rows[jCount][iCount];
                }
                sparkColName.push(name);
            }
            if (dWagon.table.columns[iCount].roles['Barchart']) {
                var col = dWagon.table.columns[iCount].queryName.replaceAll('"', '\'');
                var name = col.substring(col.indexOf('.') + 1, col.length).replaceAll('"', '\'');
                if (col.lastIndexOf(')') === (col.length - 1)) {
                    name = col.substring(col.indexOf('.') + 1, col.lastIndexOf(')')).replaceAll('"', '\'');
                ...
                if (value === null) {
                    ...
                    barObj[name] = dWagon.table.rows[jCount][iCount];
                }
                barColName.push(name);
            }
            if (dWagon.table.columns[iCount].roles['Gauge']) {
                var col = dWagon.table.columns[iCount].queryName.replaceAll('"', '\'');
                var name = col.substring(col.indexOf('.') + 1, col.length).replaceAll('"', '\'');
                if (col.lastIndexOf(')') === (col.length - 1)) {
                    name = col.substring(col.indexOf('.') + 1, col.lastIndexOf(')')).replaceAll('"', '\'');
                ...
                if (value === null) {
                    ...
                    gaugeObj[name] = dWagon.table.rows[jCount][iCount];
                }
                gaugeColName.push(name);
        ...
        data.push(obj);
        tempSparkData.push(sparkObj);
        tempBarData.push(barObj);
        tempGaugeData.push(gaugeObj)
    }
    if (tempSparkData && sparkColName.length === 3) {
        config.hasSparkLine = true;
        config.sparkColumnHeader = sparkColName;
        var j = 0; var i = 0;
        var currCat = tempSparkData[0][sparkColName[0]];
        sparkData[0] = [];
        sparkData[0][0] = tempSparkData[0];
        for (var count = 1; count < tempSparkData.length; count++) {
            if (tempSparkData[count][sparkColName[0]] === currCat) {
                j++;
                sparkData[i][j] = tempSparkData[count];
            }
            else {
                i++; j = 0;
                sparkData[i] = [];
                sparkData[i][j] = tempSparkData[count];
                currCat = tempSparkData[count][sparkColName[0]]
            }

        }
    }
    if (tempBarData && barColName.length === 3) {
        config.hasbarChart = true;
        config.barColumnHeader = barColName;
        var j = 0; var i = 0;
        var currCat = tempBarData[0][barColName[0]];
        barData[0] = [];
        barData[0][0] = tempBarData[0];
        for (var count = 1; count < tempBarData.length; count++) {
            if (tempBarData[count][barColName[0]] === currCat) {
                j++;
                barData[i][j] = tempBarData[count];
            }
            else {
                i++; j = 0;
                barData[i] = [];
                barData[i][j] = tempBarData[count];
                currCat = tempBarData[count][barColName[0]]
            }

        }
    }
    if (gaugeColName.length > 0) {
        config.hasGauge = true;
        for (var i = 0; i < tempGaugeData.length; i++) {
            gaugeData.push({
                'colorActual': gridFormatters.gaugeActualColor,
                'colorComparison': gridFormatters.gaugeComparisonColor,
                'titleLabel': '',
                'subTitleLabel': '',
                'showLabel': true,
                'states': [],
                'min': gaugeColName.length > 3 ? tempGaugeData[i][gaugeColName[3]] : 0,
                'max': gaugeColName.length > 2 ? tempGaugeData[i][gaugeColName[2]] : tempGaugeData[i][gaugeColName[0]] * 2,
                'value': 0,
                'target': gaugeColName.length > 1 ? tempGaugeData[i][gaugeColName[1]] : 0,
                'comparison': 0,
                'actual': tempGaugeData[i][gaugeColName[0]],
                'percentage': 0,
                'trendValue1': 0,
                'trendValue2': 0,
                'actualFormat': '',
    ...
    config.data = data;
    config.sparkData = sparkData;
    config.barData = barData;
    config.tableColumnHeader = [];
    config.gaugeData = gaugeData;
    for (iCount = 0; iCount < dWagon.table.columns.length; iCount++) {
        obj = {};
        ...
        obj.format = null;
        if (dWagon.table.columns[iCount].format && sort_type != 'object')
            obj.format = valueFormatter.create({ format: dWagon.table.columns[iCount].format });
        obj.roles = dWagon.table.columns[iCount].roles;
        config.tableColumnHeader.push(obj);
    }
    ...
    if (MAQ.gridName.length > 0) {
        ...
        if (-1 < gridObjectPosition) {
            ...
            if (oCurrentGridConfiguration.serverGrid.enabled) {
                ...
                for (columnCounter in oCurrentGridConfiguration.tableColumnHeader) {
                    if (oCurrentGridConfiguration.tableColumnHeader[columnCounter].name === fieldName && oCurrentGridConfiguration.tableColumnHeader[columnCounter].sortType) {
                        if (oCurrentGridConfiguration.tableColumnHeader[columnCounter].sortAttribute) {
                            fieldName = oCurrentGridConfiguration.tableColumnHeader[columnCounter].sortAttribute;
                        }
                        sortType = oCurrentGridConfiguration.tableColumnHeader[columnCounter].sortType;
                    }
                }
...
MAQ.CreatePaginationControl = function (GridConfiguration) {
    var paginationSpaceRow = GridConfiguration.tblFoot.insertRow(0),
        ...
        oJumpLast = document.createElement("div"),
        oJumpFirst = document.createElement("div"),
    ...
    if (GridConfiguration.fixedHeaderEnd) {
        if (GridConfiguration.containerObject.clientWidth - GridConfiguration.tblBody.clientWidth >= GridConfiguration.tblBodyRight.clientWidth) {
            ...
            rightSpaceCell.colSpan = GridConfiguration.tableColumnHeader.length;
    ...
    spaceCell.colSpan = GridConfiguration.tableColumnHeader.length;
    MAQUtility.addClass(spaceCell, "jsonPaginationMargin");

    // Cell containing the pagination content
    cell.colSpan = GridConfiguration.tableColumnHeader.length;
    ...
    if (!GridConfiguration.isWin8App) {
        ...
        if (0 === GridConfiguration.currentPage) {
            $(oJumpFirst).html('<span id="' + GridConfiguration.gridName + '_First" class="first cur-pointer" click-disabled" active="1" onclick="MAQ.goFirst(this,\'' + GridConfiguration.gridName + '\')">&laquo</span>');
            ...
            $(oJumpFirst).html('<span id="' + GridConfiguration.gridName + '_First" class="first cur-pointer"  active="1" onclick="MAQ.goFirst(this,\'' + GridConfiguration.gridName + '\')">&laquo</span>');
        ...
        if (iLastPage - 1 !== GridConfiguration.currentPage) {
            ...
            $(oJumpLast).html('<span id="' + GridConfiguration.gridName + '_Last" class="last cur-pointer" active="0" onclick="MAQ.goLast(this,\'' + GridConfiguration.gridName + '\')">&raquo</span>');

        }
    } else {
        MSApp.execUnsafeLocalFunction(function () {
            if (0 === GridConfiguration.currentPage) {
                WinJS.Utilities.setInnerHTML(oJumpFirst, '<span id="' + GridConfiguration.gridName + '_First" class="first cur-pointer" click-disabled" active="1" width="24" onclick="MAQ.goFirst(this,\'' + GridConfiguration.gridName + '\')">&laquo</span>');
                ...
                WinJS.Utilities.setInnerHTML(oJumpFirst, '<span id="' + GridConfiguration.gridName + '_First" class="first cur-pointer" active="1" width="24" onclick="MAQ.goFirst(this,\'' + GridConfiguration.gridName + '\')">&laquo</span>');
            ...
            WinJS.Utilities.setInnerHTML(oJumpLast, '<span id="' + GridConfiguration.gridName + '_Last" class="last cur-pointer" active="0" width="24" onclick="MAQ.goLast(this,\'' + GridConfiguration.gridName + '\')">&raquo</span>');
        });
    }
    oJumpFirst.className = "PaginationPrevArrowDiv";
    ...
    oJumpLast.className = "PaginationNextArrowDiv";

    oPaginationContainer.className = "jsonGridFooter";
    oPaginationContainer.appendChild(oJumpFirst);
    ...
    oPaginationContainer.appendChild(oJumpLast);
...
MAQ.setDrillDown = function (sCellValue, iCurrentRow, oGridConfiguration, bEndRow) {
    ...
    for (iDrillIterator = 0; iDrillIterator < oGridConfiguration.tableColumnHeader.length; iDrillIterator++) {
        if (bEndRow) {
            oDrillObject.push(oGridConfiguration.endRow.data[oGridConfiguration.tableColumnHeader[iDrillIterator].name] || "");
        } else {
            oDrillObject.push(oGridConfiguration.data[iCurrentRow][oGridConfiguration.tableColumnHeader[iDrillIterator].name] || "");
        }
    }
    ...
    if (oGridConfiguration.fixedHeaderEnd) {
        ...
        oDrillCell.setAttribute("colspan", oGridConfiguration.tableColumnHeader.length - oGridConfiguration.fixedHeaderEnd + iDrillDownCounter);
    } else {
        oDrillCell.setAttribute("colspan", oGridConfiguration.tableColumnHeader.length + iDrillDownCounter);
    }
...
MAQ.CreateHTMLTableRow = function (GridConfiguration) {
    ...
    var startIndex, endIndex, cell = null,
        ...
        numberOfColumns = GridConfiguration.tableColumnHeader.length,
        oCurrentObject = this,
        sIsNegativeLevel = "0",
        iInnerCounter = 0,
    ...
    for (this.rowPosition = startIndex; this.rowPosition < endIndex; this.rowPosition += 1) {
        ...
        for (cellCounter = 0; cellCounter < numberOfColumns; cellCounter++) {
            if (GridConfiguration.tableColumnHeader[cellCounter].roles['Values']) {
                ...
                cell.style.textAlign = GridConfiguration.tableColumnHeader[cellCounter].align;

                // To add which report to invoke
                if (GridConfiguration.tableColumnHeader[cellCounter]["data-name"]) {
                    cell.setAttribute("data-name", GridConfiguration.tableColumnHeader[cellCounter]["data-name"]);
                }

                // Doing this to avoid overlapping of last column values and the scroll bar
                if (GridConfiguration.tableColumnHeader[cellCounter].noOverlap) {
                ...
                if (GridConfiguration.tableColumnHeader[cellCounter].style) {
                    MAQ.applyStyleToObject(cell, GridConfiguration.tableColumnHeader[cellCounter].style);
                }

                if (!GridConfiguration.tableColumnHeader[cellCounter].formatter) {
                    if (GridConfiguration.tableColumnHeader[cellCounter].format) {
                        if (GridConfiguration.tableColumnHeader[cellCounter].trimOnOverflow) {
                            sReturnValue = GridConfiguration.tableColumnHeader[cellCounter].format.format(parseFloat(MAQ.getAdjustedRowChunk((GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name] || oGridConstants.sNA), GridConfiguration.tableColumnHeader[cellCounter].style.width)));
                        } else {
                            if (GridConfiguration.tableColumnHeader[cellCounter].trimOnOverflowAndShowToolTip) {
                                sReturnValue = GridConfiguration.tableColumnHeader[cellCounter].format.format(parseFloat(MAQ.getAdjustedRowChunkAndToolTip((GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name] || oGridConstants.sNA), GridConfiguration.tableColumnHeader[cellCounter].style.width)));
                            } else {
                                sReturnValue = GridConfiguration.tableColumnHeader[cellCounter].format.format(parseFloat((GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name] || oGridConstants.sNA)));
                            }
                        }
                    }
                    else {
                        if (GridConfiguration.tableColumnHeader[cellCounter].trimOnOverflow) {
                            sReturnValue = MAQ.getAdjustedRowChunk((GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name] || oGridConstants.sNA), GridConfiguration.tableColumnHeader[cellCounter].style.width);
                        } else {
                            if (GridConfiguration.tableColumnHeader[cellCounter].trimOnOverflowAndShowToolTip) {
                                sReturnValue = MAQ.getAdjustedRowChunkAndToolTip((GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name] || oGridConstants.sNA), GridConfiguration.tableColumnHeader[cellCounter].style.width);
                            } else {
                                sReturnValue = (GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name] || oGridConstants.sNA);
                            }
                        }
                    }
                } else {
                    sReturnValue = "";
                    if (window[GridConfiguration.tableColumnHeader[cellCounter].formatter] || GridConfiguration.tableColumnHeader[cellCounter].formatter === "trimOnOverflowAndShowToolTip") {
                        switch (GridConfiguration.tableColumnHeader[cellCounter].formatter) {
                            case "parseCustomBarChart":
                                iCount = 0, iTotal = 0, fMaxValue = 1, fCurrent = 0, sFieldName = GridConfiguration.tableColumnHeader[cellCounter].name;
                                ...
                                oFormatterOptions = {
                                    maxValue: fMaxValue,
                                    field: sFieldName,
                                    numberFormatter: GridConfiguration.tableColumnHeader[cellCounter].chartValueFormatter,
                                    fCell: (cell.style.width || oGridConstants.sDefaultWidth)
                                };

                                sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.data[this.rowPosition], oFormatterOptions);
                                break;
                            case "parseDealValue":
                                sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.data[this.rowPosition], GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name]);
                                break;
                            case "parsePastDueDeals":
                                sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.data[this.rowPosition], GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.dataConfiguration.maxConfig);
                                break;
                            case "parseOpportunitiesTrack":
                                sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.data[this.rowPosition], GridConfiguration.dataConfiguration.maxConfig, cell.style.width || "");
                                break;
                            case "parseOpportunitiesMissing":
                                sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.data[this.rowPosition], GridConfiguration.dataConfiguration.maxConfig, cell.style.width || "");
                                break;
                            case "parseQuotaCoverage":
                                sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.data[this.rowPosition], GridConfiguration.dataConfiguration.maxConfig);
                                break;
                            case "calculatePercentByTotal":
                                oFormatterOptions = {
                                    ...
                                    barColor: GridConfiguration.tableColumnHeader[2].barColors[this.rowPosition],
                                    barColors: GridConfiguration.tableColumnHeader[2].barColors,
                                    dataSeries: GridConfiguration.data
                                };
                                sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.data[this.rowPosition][GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.data[this.rowPosition], GridConfiguration.gridName, GridConfiguration.tableColumnHeader[cellCounter].name, oFormatterOptions);
                            ...
                            case "customPipelineBar":
                                sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.data[this.rowPosition], GridConfiguration.data, GridConfiguration.endRow.data);
                                break;
                            ...
                            default:
                                oFormatterOptions = {
                                    ...
                                    fCell: cell.style.width || "",
                                    cellCounter: this.rowPosition,
                                    headerProperties: GridConfiguration.tableColumnHeader[cellCounter],
                                    dataSeries: GridConfiguration.data,
                ...
                if (GridConfiguration.tableColumnHeader[cellCounter].category === "ImageUrl") {
                    let img = document.createElement("img");
                    img.src = sReturnValue;
                    img.style.height = "100%";
                    cell.appendChild(img);
                }
                else if (GridConfiguration.tableColumnHeader[cellCounter].category === "WebUrl") {
                    let link = document.createElement("a");
                    link.innerHTML = sReturnValue;
        ...
        if (GridConfiguration.hasSparkLine) {
            for (cellCounter = 0; cellCounter < numberOfColumns; cellCounter++) {
                if (GridConfiguration.tableColumnHeader[cellCounter].roles['SparkLine']) {
                    if (GridConfiguration.fixedHeaderEnd && parseInt(GridConfiguration.fixedHeaderEnd) <= cellCounter) {
                        cell = oRowRight.insertCell(iCellCounterRight++);
                    } else {
                        cell = row.insertCell(iCellCounter++);
            ...
            if (GridConfiguration.tableColumnHeader[cellCounter].roles['SparkLine']) {
                ...
                var svg = d3.select(cell)
                    .append("svg:svg");
                let colNames = GridConfiguration.sparkColumnHeader;
                var sparkData = GridConfiguration.sparkData;
                while (sReturnValue != sparkData[count][0][colNames[0]])
                    count++;
                for (var i = 0; i < sparkData[count].length; i++) {
                    valueData[i] = sparkData[count][i][colNames[2]];
                    tooltipData.push({
                        "catColumn": colNames[1],
                        "valueColumn": colNames[2],
                        "category": sparkData[count][i][colNames[1]],
                        "value": sparkData[count][i][colNames[2]]
                    })
                }
                createSparkLine(svg, div, valueData, tooltipData,
                    GridConfiguration.tableColumnHeader[cellCounter].width, GridConfiguration.sparklineColor, 1, 'black', 'white');
            }
        ...
        if (GridConfiguration.hasbarChart) {
            for (cellCounter = 0; cellCounter < numberOfColumns; cellCounter++) {
                if (GridConfiguration.tableColumnHeader[cellCounter].roles['Barchart']) {
                    if (GridConfiguration.fixedHeaderEnd && parseInt(GridConfiguration.fixedHeaderEnd) <= cellCounter) {
                        cell = oRowRight.insertCell(iCellCounterRight++);
                    } else {
                        cell = row.insertCell(iCellCounter++);
            ...
            if (GridConfiguration.tableColumnHeader[cellCounter].roles['Barchart']) {
                ...
                var svg = d3.select(cell)
                    .append("svg:svg");
                let colNames = GridConfiguration.barColumnHeader;
                var barData = GridConfiguration.barData;
                while (sReturnValue != barData[count][0][colNames[0]])
                    count++;
                for (var i = 0; i < barData[count].length; i++) {
                    data.push({
                        'x': barData[count][i][colNames[1]],
                        'y': barData[count][i][colNames[2]]
                    })
                    tooltipData.push({
                        "catColumn": colNames[1],
                        "valueColumn": colNames[2],
                        "category": barData[count][i][colNames[1]],
                        "value": barData[count][i][colNames[2]]
                    })
                }
                createBarChart(svg, div, data, tooltipData, GridConfiguration.tableColumnHeader[cellCounter].width, GridConfiguration.barchartColor, 1, 'black', 'white');
            }
            else {
                // Format cell for drill down end
                if (!GridConfiguration.isWin8App) {
        ...
        if (GridConfiguration.hasGauge) {
            for (cellCounter = 0; cellCounter < numberOfColumns; cellCounter++) {
                if (GridConfiguration.tableColumnHeader[cellCounter].roles['Gauge']) {
                    if (GridConfiguration.fixedHeaderEnd && parseInt(GridConfiguration.fixedHeaderEnd) <= cellCounter) {
                        cell = oRowRight.insertCell(iCellCounterRight++);
                    } else {
                        cell = row.insertCell(iCellCounter++);
                    ...
                    createLinearGauge(svg, div, div, GridConfiguration.gaugeData[this.rowPosition],
                        GridConfiguration.tableColumnHeader[cellCounter].width, 30);
    ...
    if (GridConfiguration.endRow.enableEndRow) {
        ...
        if (!isNaN(GridConfiguration.endRow.endRowPosition) && GridConfiguration.endRow.endRowPosition >= -1 && GridConfiguration.endRow.endRowPosition < GridConfiguration.data.length) {
            ...
            for (cellCounter = 0; cellCounter < numberOfColumns; cellCounter++) {
                ...
                cell.style.textAlign = GridConfiguration.tableColumnHeader[cellCounter].align;
                if (GridConfiguration.endRow.includeFormatters && GridConfiguration.endRow.includeFormatters[GridConfiguration.tableColumnHeader[cellCounter].name]) {
                    switch (GridConfiguration.tableColumnHeader[cellCounter].formatter) {

                        case "parseDealValue":
                            sReturnValue = window[GridConfiguration.endRow.includeFormatters[GridConfiguration.tableColumnHeader[cellCounter].name]](GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.endRow.data, GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name]);
                            break;
                        case "parsePastDueDeals":
                            sReturnValue = window[GridConfiguration.endRow.includeFormatters[GridConfiguration.tableColumnHeader[cellCounter].name]](GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.endRow.data, GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name]);
                            break;
                        case "parseOpportunitiesTrack":
                            sReturnValue = window[GridConfiguration.endRow.includeFormatters[GridConfiguration.tableColumnHeader[cellCounter].name]](GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.endRow.data, GridConfiguration.dataConfiguration.maxConfig, cell.style.width || oGridConstants.sDefaultWidth);
                            break;
                        case "parseOpportunitiesMissing":
                            sReturnValue = window[GridConfiguration.endRow.includeFormatters[GridConfiguration.tableColumnHeader[cellCounter].name]](GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.endRow.data, GridConfiguration.dataConfiguration.maxConfig, cell.style.width || oGridConstants.sDefaultWidth);
                            break;
                        case "parseQuotaCoverage":
                            sReturnValue = window[GridConfiguration.endRow.includeFormatters[GridConfiguration.tableColumnHeader[cellCounter].name]](GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.endRow.data, GridConfiguration.dataConfiguration.maxConfig);
                            break;
                        case "customPipelineBar":
                            sReturnValue = window[GridConfiguration.tableColumnHeader[cellCounter].formatter](GridConfiguration.endRow.data, GridConfiguration.data, GridConfiguration.endRow.data);
                            break;
                        default:
                            sReturnValue = window[GridConfiguration.endRow.includeFormatters[GridConfiguration.tableColumnHeader[cellCounter].name]](GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name], GridConfiguration.endRow.data, GridConfiguration.gridName, GridConfiguration.tableColumnHeader[cellCounter].name);
                            break;
                    }
                } else {
                    sReturnValue = (GridConfiguration.endRow.data[GridConfiguration.tableColumnHeader[cellCounter].name] || oGridConstants.sNA);
                }
...
MAQ.CreateHTMLTableWithHeader = function (GridConfiguration) {
    var tHead = GridConfiguration.tblHead,
        ...
        iParentCounter = 0,
        numberOfHeaderColumns = GridConfiguration.tableColumnHeader.length,
    ...
    for (iLoopCounter = 0; iLoopCounter < (numberOfHeaderColumns); iLoopCounter += 1) {
        if (GridConfiguration.tableColumnHeader[iLoopCounter].roles['Values']) {
            if (GridConfiguration.fixedHeaderEnd && parseInt(GridConfiguration.fixedHeaderEnd) <= iLoopCounter) {
                cell = oRowRight.insertCell(iCellCounterRight++);
            } else {
                cell = row.insertCell(iCellCounter++);
            }
            cell.setAttribute("id", GridConfiguration.tableColumnHeader[iLoopCounter].id || "jsonGridHeader_" + iLoopCounter);
            MAQUtility.addClass(cell, GridConfiguration.tableColumnHeader[iLoopCounter].headerClassName);
            MAQUtility.addClass(cell, "jsonGridHeader");
            if (GridConfiguration.tableColumnHeader[iLoopCounter].style) {
                if (iLoopCounter === numberOfHeaderColumns - 1) {
                    cell.style.width = (GridConfiguration.scrolling.enabled) ? (GridConfiguration.tblBody.rows[0].cells[iLoopCounter].clientWidth - 8) + "px" : GridConfiguration.tableColumnHeader[iLoopCounter].style.width;
                } else {
                    cell.style.width = (GridConfiguration.scrolling.enabled) ? (GridConfiguration.tblBody.rows[0].cells[iLoopCounter].clientWidth - 30) + "px" : GridConfiguration.tableColumnHeader[iLoopCounter].style.width;
                }
                cell.style.textAlign = GridConfiguration.tableColumnHeader[iLoopCounter].style.textAlign;
            }
            ...
            if (GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                cell.setAttribute("onclick", 'MAQ.sortJsonGrid(this,"' + GridConfiguration.container + '_Grid","' + GridConfiguration.tableColumnHeader[iLoopCounter].name + '")');
                cell.style.cursor = "pointer";
                if (GridConfiguration.tableColumnHeader[iLoopCounter].name === GridConfiguration.gridSort.sortby) {
                    ...
                    if (GridConfiguration.hiddenContainer) {
                        GridConfiguration.hiddenContainer.setAttribute("data-sortKey", GridConfiguration.tableColumnHeader[iLoopCounter].sortKey);
                    }
                } else {
                    cell.setAttribute("sortorder", GridConfiguration.gridSort.sortorder);
                }
                cell.setAttribute("sortKey", GridConfiguration.tableColumnHeader[iLoopCounter].sortKey);
            } else {
                cell.style.cursor = "default";
            }
            if (!GridConfiguration.gridSort.sortby && GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                GridConfiguration.gridSort.sortby = GridConfiguration.tableColumnHeader[iLoopCounter].name;
            }
            if (GridConfiguration.tableColumnHeader[iLoopCounter].name === GridConfiguration.gridSort.sortby && GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                ...
                $(cell).html("<span class='ColumnText'>" + GridConfiguration.tableColumnHeader[iLoopCounter].columnText + "</span>");
                if (GridConfiguration.gridSort.sortorder === "asc") {
                    $(cell).append("<span class='SortIndicator sort" + GridConfiguration.tableColumnHeader[iLoopCounter].name.replace(/[!\"#$%&'\(\)\*\+,\.\/:;<=>\?\@\[\\\]\^`\{\|\}~ ]/g, '_') + "Hand'>" + '<span class="asc"></span>' + "</span>");
                } else {
                    $(cell).append("<span class='SortIndicator sort" + GridConfiguration.tableColumnHeader[iLoopCounter].name.replace(/[!\"#$%&'\(\)\*\+,\.\/:;<=>\?\@\[\\\]\^`\{\|\}~ ]/g, '_') + "Hand'>" + '<span class="desc"></span>' + "</span>");
                }
            } else if (GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                $(cell).html("<span class='ColumnText'>" + GridConfiguration.tableColumnHeader[iLoopCounter].columnText + "</span>");
                if (GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                    $(cell).append("<span class='itemHide SortIndicator sort" + GridConfiguration.tableColumnHeader[iLoopCounter].name.replace(/[!\"#$%&'\(\)\*\+,\.\/:;<=>\?\@\[\\\]\^`\{\|\}~ ]/g, '_') + "Hand'>" + '<span class="asc"></span>' + "</span>");
                }
            } else {
                $(cell).html("<span class='ColumnText'>" + GridConfiguration.tableColumnHeader[iLoopCounter].columnText + "</span>");
            }
    ...
    if (GridConfiguration.hasSparkLine) {
        var count = 0, cellText;
        for (iLoopCounter = 0; iLoopCounter < (numberOfHeaderColumns); iLoopCounter += 1) {
            if (GridConfiguration.tableColumnHeader[iLoopCounter].roles['SparkLine']) {
                count++;
                if (count > 2)
                    break;
                if (count === 2)
                    ...
                    cellText = GridConfiguration.tableColumnHeader[iLoopCounter].columnText;
                if (GridConfiguration.fixedHeaderEnd && parseInt(GridConfiguration.fixedHeaderEnd) <= iLoopCounter) {
                    cell = oRowRight.insertCell(iCellCounterRight++);
                } else {
                    cell = row.insertCell(iCellCounter++);
                ...
                if (GridConfiguration.tableColumnHeader[iLoopCounter].name === GridConfiguration.gridSort.sortby && GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                    ...
                    $(cell).html("<span class='ColumnText'>" + cellText + "</span>");
                    if (GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                        $(cell).append("<span class='itemHide SortIndicator sort" + GridConfiguration.tableColumnHeader[iLoopCounter].name.replace(/[!\"#$%&'\(\)\*\+,\.\/:;<=>\?\@\[\\\]\^`\{\|\}~ ]/g, '_') + "Hand'>" + '<span class="asc"></span>' + "</span>");
                    }
                } else {
    ...
    if (GridConfiguration.hasbarChart) {
        var count = 0, cellText;
        for (iLoopCounter = 0; iLoopCounter < (numberOfHeaderColumns); iLoopCounter += 1) {
            if (GridConfiguration.tableColumnHeader[iLoopCounter].roles['Barchart']) {
                count++;
                if (count > 2)
                    break;
                if (count === 2)
    ...
    if (GridConfiguration.hasGauge) {
        for (iLoopCounter = 0; iLoopCounter < (numberOfHeaderColumns); iLoopCounter += 1) {
            if (GridConfiguration.tableColumnHeader[iLoopCounter].roles['Gauge']) {
                if (GridConfiguration.fixedHeaderEnd && parseInt(GridConfiguration.fixedHeaderEnd) <= iLoopCounter) {
                    cell = oRowRight.insertCell(iCellCounterRight++);
                } else {
                    cell = row.insertCell(iCellCounter++);
                ...
                if (GridConfiguration.tableColumnHeader[iLoopCounter].name === GridConfiguration.gridSort.sortby && GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                    ...
                    $(cell).html("<span class='ColumnText'>" + "Linear Gauge" + "</span>");
                    if (GridConfiguration.tableColumnHeader[iLoopCounter].sortable) {
                        $(cell).append("<span class='itemHide SortIndicator sort" + GridConfiguration.tableColumnHeader[iLoopCounter].name.replace(/[!\"#$%&'\(\)\*\+,\.\/:;<=>\?\@\[\\\]\^`\{\|\}~ ]/g, '_') + "Hand'>" + '<span class="asc"></span>' + "</span>");
                    }
                } else {
...
MAQ.CreateHTMLTable = function (GridConfiguration) {
    ...
    if (GridConfiguration.fixedHeaderEnd) {
        ...
        if (GridConfiguration.tableColumnHeader.length === parseInt(GridConfiguration.fixedHeaderEnd)) {
...
MAQ.JsonGrid = function (gridConfigurationOptions) {
    ...
    if (this.containerObject) {
        ...
        if (this.gridOptions.drillDown.enabled) {
            ...
            if (!(!isNaN(this.gridOptions.drillDown.columnId) && this.gridOptions.drillDown.columnId >= -1 && this.gridOptions.drillDown.columnId <= this.gridOptions.tableColumnHeader.length)) {
        ...
        for (this.LoopCounter = 0; this.LoopCounter < this.gridOptions.tableColumnHeader.length; this.LoopCounter += 1) {
            if (!(this.gridOptions.data[0].hasOwnProperty([this.gridOptions.tableColumnHeader[this.LoopCounter].name].toString()) || "DUMMY" === this.gridOptions.tableColumnHeader[this.LoopCounter].name)) {
                return false;
            }

            // Add color configuration for custom bar chart column
            if (this.gridOptions.tableColumnHeader[this.LoopCounter].formatter && this.gridOptions.tableColumnHeader[this.LoopCounter].formatter === "parseCustomBarChart") {
                iTotal = this.gridOptions.data.length;
                if (this.gridOptions.tableColumnHeader[this.LoopCounter].chartColor && iTotal === this.gridOptions.tableColumnHeader[this.LoopCounter].chartColor.length) {
                    for (iCount = 0; iCount < iTotal; iCount++) {
                        ...
                        this.gridOptions.data[iCount].colorStyle.columnName.push(this.gridOptions.tableColumnHeader[this.LoopCounter].name);
                        this.gridOptions.data[iCount].colorStyle.color.push(this.gridOptions.tableColumnHeader[this.LoopCounter].chartColor[iCount]);
                    }
                }
            }
        }
        ...
        iColumnHeaderLength = this.gridOptions.tableColumnHeader.length;
        ...
        if (this.gridOptions.endRow.isTotalRow) {
            for (iColumnIterator = 0; iColumnIterator < iColumnHeaderLength; iColumnIterator++) {
                for (iIterator = 0; iIterator < iDataLength; iIterator++) {
                    if (this.gridOptions.endRow.columnsExcluded && this.gridOptions.endRow.columnsExcluded.indexOf(this.gridOptions.tableColumnHeader[iColumnIterator].name) < 0) {
                        if ("undefined" === typeof this.gridOptions.endRow.data[this.gridOptions.tableColumnHeader[iColumnIterator].name]) {
                            this.gridOptions.endRow.data[this.gridOptions.tableColumnHeader[iColumnIterator].name] = 0;
                        } else if (isNaN(this.gridOptions.endRow.data[this.gridOptions.tableColumnHeader[iColumnIterator].name])) {
                            this.gridOptions.endRow.data[this.gridOptions.tableColumnHeader[iColumnIterator].name] = oGridConstants.sNA;
                            break;
                        }
                        nCurrentValue = parseFloat(this.gridOptions.data[iIterator][this.gridOptions.tableColumnHeader[iColumnIterator].name]);
                        if (isNaN(nCurrentValue)) {
                            nCurrentValue = 0;
                        }
                        this.gridOptions.endRow.data[this.gridOptions.tableColumnHeader[iColumnIterator].name] += nCurrentValue;
                    }
                }
            }
            nCurrentValue = 0;
