...
function renderMAQVisual(objDataView, formatOption, valueFormatter) {
    ...
    var agreeConfig = {
        ...
        "xAxis": {
            ...
            "labels": {
                ...
                "rotation": 0,
                "style": {
                    "fill": formatOption.xAxisFontColor,
        ...
        "yAxis": {
            ...
            "labels": {
                ...
                "rotation": 0,
                "style": {
                    "fill": formatOption.yAxisFontColor,
    ...
    agreeConfig.xAxis.labels.style.fontSize = (formatOption.xAxisFontSize * 30 / 40) + "px";
    agreeConfig.yAxis.labels.style.fontSize = (formatOption.yAxisFontSize * 30 / 40) + "px";

    var obj, data = [],
        jCount, iCount, keys;

    if (!objDataView || !objDataView.metadata || !objDataView.metadata.columns || objDataView.metadata.columns.length === 0
        || !insertData.categorical || !insertData.categorical.categories || !insertData.categorical.categories[0].values
        || !objDataView.table || !objDataView.table.rows || objDataView.table.rows.length === 0) {
        d3.select("#agreementChart").attr({ "style": null });
        d3.select("#agreementChart").append("svg").attr({ "width": window.frames.innerWidth + "px", "height": window.frames.innerHeight + "px" });
        d3.select("svg").append("text")
            .attr({ "x": (window.frames.innerWidth / 2) + "px", "y": (window.frames.innerHeight / 2) + "px", "font-size": "20px", "text-anchor": "middle" })
    ...
    var parentPresentCounter = 0;
    var childPresentCounter = 0;
    var childMapper = 0;
    var parentMapper = -1;
    var widthMap = 0;
    var maxParent = 0;
    var parentList = [];
    var inValidColumnCounter = 0;
    var inValidDataCounter = 0;
    var totalRowCount = objDataView.table.rows.length;
    ...
    for (var i = 1; i <= 5; i++) {
        ...
        var parRecord = "parent" + (i);
        var chiRecord = "child" + (i);

        agreeConfig.series[parentMapper + i] = objParent;
        if (roles[parRecord].length > 0) {
            inValidDataCounter = 0;
            parentList.push(i);
            maxParent = i;
            agreeConfig.series[parentMapper + i].name = objDataView.metadata.columns[roles[parRecord][0]].displayName;
            agreeConfig.series[parentMapper + i].formatter = valueFormatter.create({ format: objDataView.metadata.columns[roles[parRecord][0]].format });
            for (j = 0; j < totalRowCount; j++) {
                if (!(typeof (objDataView.table.rows[j][roles[parRecord][0]]) === "string" || typeof (objDataView.table.rows[j][roles[parRecord][0]]) === "Object")) {
                    agreeConfig.series[parentMapper + i].data.push(objDataView.table.rows[j][roles[parRecord][0]]);
                } else {
                    agreeConfig.series[parentMapper + i].data.push("");
                    inValidDataCounter++;
                }
            }
            inValidColumnCounter = inValidDataCounter === totalRowCount ? inValidColumnCounter + 1 : inValidColumnCounter;
            parentPresentCounter++;
            widthMap++;

        }

        agreeConfig.series[childMapper + i] = objChild;
        if (roles[chiRecord].length > 0 && roles[parRecord].length > 0) {
            inValidDataCounter = 0;
            agreeConfig.series[childMapper + i].name = objDataView.metadata.columns[roles[chiRecord][0]].displayName;
            agreeConfig.series[childMapper + i].formatter = valueFormatter.create({ format: objDataView.metadata.columns[roles[chiRecord][0]].format });
            for (j = 0; j < totalRowCount; j++) {
                if (!(typeof (objDataView.table.rows[j][roles[chiRecord][0]]) === "string" || typeof (objDataView.table.rows[j][roles[chiRecord][0]]) === "object")) {
                    agreeConfig.series[childMapper + i].data.push(objDataView.table.rows[j][roles[chiRecord][0]]);
                } else {
                    agreeConfig.series[childMapper + i].data.push("");
                    inValidDataCounter++;
                }
            }
            inValidColumnCounter = inValidDataCounter === totalRowCount ? inValidColumnCounter + 1 : inValidColumnCounter;
    ...
    if (parentPresentCounter === 0 && childPresentCounter !== 0) {
        d3.select("#agreementChart").attr({ "style": null });
        d3.select("#agreementChart").append("svg").attr({ "width": window.frames.innerWidth + "px", "height": window.frames.innerHeight + "px" });
        d3.select("svg").append("text")
            .attr({ "x": (window.frames.innerWidth / 2) + "px", "y": (window.frames.innerHeight / 2) + "px", "font-size": "20px", "text-anchor": "middle" })
            .text("Enter the data in corresponding column field to load the visual");
        return;
    }
    else if (parentPresentCounter === 0 && childPresentCounter === 0) {
        d3.select("#agreementChart").attr({ "style": null });
        d3.select("#agreementChart").append("svg").attr({ "width": window.frames.innerWidth + "px", "height": window.frames.innerHeight + "px" });
        d3.select("svg").append("text")
            .attr({ "x": (window.frames.innerWidth / 2) + "px", "y": (window.frames.innerHeight / 2) + "px", "font-size": "20px", "text-anchor": "middle" })
            .text("Enter the data in any column field to load the visual");
        return;
    }

    if (inValidColumnCounter === widthMap) {
        d3.select("#agreementChart").attr({ "style": null });
        d3.select("#agreementChart").append("svg").attr({ "width": window.frames.innerWidth + "px", "height": window.frames.innerHeight + "px" });
        d3.select("svg").append("text")
            .attr({ "x": (window.frames.innerWidth / 2) + "px", "y": (window.frames.innerHeight / 2) + "px", "font-size": "20px", "text-anchor": "middle" })
            .text("Invalid Data to Display");
        return;
    }
    else if (inValidColumnCounter !== 0) {
        d3.select("#agreementChart").attr({ "style": null });
        d3.select("#agreementChart").append("svg").attr({ "width": window.frames.innerWidth + "px", "height": window.frames.innerHeight + "px" });
        d3.select("svg").append("text")
            .attr({ "x": (window.frames.innerWidth / 2) + "px", "y": (window.frames.innerHeight / 2) + "px", "font-size": "20px", "text-anchor": "middle" })
            .text("Inconsistent Data to Display");
        return;
    }
    ...
    while (i < 10) {
        i++;
        agreeConfig.series.pop();
        agreeConfig.plotOptions.column.color.pop();
        widthMap--;
    }

    for (var maxParentCounter = maxParent; maxParentCounter >= 1; maxParentCounter--) {
        if (parentList.indexOf(maxParentCounter) === -1) {
            agreeConfig.series.splice((maxParentCounter - 1) * 2, 2);
            agreeConfig.plotOptions.column.color.splice((maxParentCounter - 1) * 2, 2);
        }
    }
    ...
    for (i = 0; i <= widthMap; i++) {
        if (!agreeConfig.series[i]) {
            agreeConfig.series[i] = objChildEmpty;
        }
    }

    for (legendName = 0; legendName < agreeConfig.series.length; legendName++) {
        if (agreeConfig.series[legendName]) {
            agreeConfig.series[legendName].enabled = getLegendEnable()[legendName];
        }
    }

    var xAxisNames = 0;
    for (xAxisNames = 0; xAxisNames < insertData.categorical.categories[0].values.length; xAxisNames++) {
        agreeConfig.xAxis.labels.series[xAxisNames] = insertData.categorical.categories[0].values[xAxisNames] === "" ? "N/A" : insertData.categorical.categories[0].values[xAxisNames];
    }

    if (!objDataView.metadata.columns[roles['categoryname'][0]] || objDataView.metadata.columns.length < 2) {
        d3.select("#agreementChart").attr({ "style": null });
        d3.select("#agreementChart").append("svg").attr({ "width": window.frames.innerWidth + "px", "height": window.frames.innerHeight + "px" });
        d3.select("svg").append("text")
            .attr({ "x": (window.frames.innerWidth / 2) + "px", "y": (window.frames.innerHeight / 2) + "px", "font-size": "20px", "text-anchor": "middle" })
    ...
    var xFormatter = valueFormatter.create({ format: objDataView.metadata.columns[roles['categoryname'][0]].format });
    var yFormatter = valueFormatter.create({ format: null });
    var parentCounter = 1;
    while (parentCounter <= 5) {
        if (roles['parent' + parentCounter] && objDataView.metadata.columns[roles['parent' + parentCounter][0]] && objDataView.metadata.columns[roles['parent' + parentCounter][0]].format) {
            yFormatter = valueFormatter.create({ format: objDataView.metadata.columns[roles['parent' + parentCounter][0]].format });
            break;
        }
        parentCounter++;
    }
    ...
    (function (agreeConfig) {
        ...
        */
        MAQ.getMinMax = function (oDataArray, min, max, fieldName) {
            ...
            if (iDataArrayLength) {
                ...
                min = Math.min(Number.MAX_VALUE, min);
                max = Math.max(Number.NEGATIVE_INFINITY, max);
                iSum = 0;
            }
            return {
                min: min,
        ...
        MAQ.getNormalized_Min_Max_Interval = function (iMin, iMax, iNumberOfAxis) {
            ...
            if (iMax - iMin === 0) {
                if (iMax === 0) {
                    ...
                    iMin = 0;
                    fInterval = ((iMax - iMin) / (iNumberOfAxis));
                }
                else {
                    iMax = 0;
                    fInterval = ((iMax - iMin) / (iNumberOfAxis));
            ...
            if (iMin == Number.MAX_VALUE && iMax == Number.NEGATIVE_INFINITY) {
                iMin = 0;
                iMax = 4;
                fInterval = 1;
            }
            ...
            if (!oParam.type) {
                ...
                if (sToolTipFunctionName) {
                    ...
                    switch (sChartType.toLowerCase()) {
                        default: oToolTip.innerHTML = '<b>' + oSeries.name + '</b><br/>' + xFormatter.format(oLabels[iSelectedIndex]) + ': ' + oSeries.formatter.format(oSeries.data[iSelectedIndex]);
                            break;
                    }
                }
            } else {
        ...
        */
        MAQ.drawLegend = function (chartConfigOptions) {
            ...
            var oLegend = chartConfigOptions.legend,
                oLegStyle, oChartStyle, oLegCStyle, oChartCStyle, sRectRule, sswapRectAttRule, sTextRule, sTextCRule, sChartRule, sChartCRule, obj;
            if (oLegend.enabled) {
                oLegend.legends = [];
                if (chartConfigOptions.series.length > 0) {
                    ...
                    if (chartConfigOptions.legend.hover.enabled) {
                        ...
                        sswapRectAttRule = '#' + chartConfigOptions.chart.renderTo + ' .MAQCharts-legend-labels>rect.MAQCharts-legend-dim';
                        ...
                        MAQ.styles.addRule(sswapRectAttRule + ', ' + sTextCRule, oLegCStyle);
        ...
        */
        ...
        @param {bflagIndicator} flagIndicator to decide xAxis or yAxis
        */
        MAQ.getAxisSpacing = function (chartConfigOptions, oAxis, sBigData, bflagIndicator) {
            'use strict';
            ...
            if (!bflagIndicator) {
                sBigData = yFormatter.format(sBigData);
            }
            fSpacing = 0;
            oAttr = {
                x: 10,
        ...
        */
        MAQ.drawAxis = function (chartConfigOptions) {
            ...
            leftSpacing = leftSpacing > 100 || isNaN(leftSpacing) ? 100 : leftSpacing;
            ...
            chartConfigOptions.svgELE.removeChild(oText);
            var iNumOfCharsAllowed = Math.ceil(leftSpacing / oDimAxis.width);
            ...
            for (iCounter = yAxisSeriesLength; yAxisLimit <= iCounter; iCounter -= 1) {
                if (bSkipPlot) {
                    ...
                    if (yAxisLabel.enabled) {
                        ...
                        oAttrLabel.text = yFormatter.format(yAxisSeries[iCounter]);
                        var sTempText = oAttrLabel.text;
                        if (sTempText.length > iNumOfCharsAllowed) {
            ...
            intervalWidth = intervalWidth < 150 ? 150 : intervalWidth;
            ...
            for (iCounter = 0; iCounter < xAxisSeries.length; iCounter += 1) {
                ...
                if (bSkipPlot) {
                    ...
                    if (xAxisLabel.enabled) {
                        iPrevX = oAttrLabel.x;
                        oAttrLabel.text = xFormatter.format(xAxisSeries[iCounter]);
                        /* Code for clipping the text to specified number of characters */
                ...
                if (bSkipPlot) {
                    ...
                    if (chartConfigOptions.plotOptions.combolinecolumn.column.multiStacked) {
                        ...
                        for (jCount = 0; jCount < iInnerSeriesLen; jCount += 1) {
                            ...
                            bSetTooltip = false;
                            oAttrInnerLabel.text = xFormatter.format(xAxisLabel.innerSeries[jCount]);
                            /* Code for clipping the text to specified number of characters */
        ...
        */
        MAQ.createColumnChart = function (chartConfigOptions) {
            ...
            d3.select("#agreementChart").style({ "overflow-y": "hidden", "overflow-x": "hidden" });
            var iMinRequiredWidth = chartConfigOptions.xAxis.labels.series.length * 155 + 200;

            if (iMinRequiredWidth > chartConfigOptions.availWidth) {
                chartConfigOptions.availWidth = iMinRequiredWidth;
                d3.select(".MAQChartsSvgRoot").style({ "width": chartConfigOptions.availWidth + 'px' });
                d3.select("#agreementChart").style("overflow-x", "auto");
            }

            if (chartConfigOptions.availHeight < 265 && chartConfigOptions.availWidth < 550) {
                chartConfigOptions.availHeight = 265;
                chartConfigOptions.availWidth = 550;
                d3.select(".MAQChartsSvgRoot").style({ "height": chartConfigOptions.availHeight + 'px', "width": chartConfigOptions.availWidth + 'px' });
                d3.select("#agreementChart").style({ "overflow-y": "auto", "overflow-x": "auto" });
            }
            if (chartConfigOptions.availWidth < 550) {
                chartConfigOptions.availWidth = 550;
                d3.select(".MAQChartsSvgRoot").style("width", chartConfigOptions.availWidth + 'px');
                d3.select("#agreementChart").style("overflow-x", "auto");
            }
            if (chartConfigOptions.availHeight < 265) {
                chartConfigOptions.availHeight = 265;
                d3.select(".MAQChartsSvgRoot").style("height", chartConfigOptions.availHeight + 'px');
                d3.select("#agreementChart").style("overflow-y", "auto");
            }
            ...
            if (oDataInfo.min || oDataInfo.max) {
                ...
                var skipValue = chartConfigOptions.availX;
                ...
                MAQ.addAttr(chartConfigOptions.xLabels, 'transform', 'translate(' + (totalBarWidth / 4) + ', 0)');
                if (!oColumnPlotOptions.stacked) {
                    //new fixedwidth
                    oBarWidth = oColumnPlotOptions.fixedWidth || oBarWidth / (parentPresentCounter + 1) ;
                }
                var oRectAttr = {
                    x: 0,
                    y: 0,
                ...
                for (iSeriesCounter = 0; iSeriesCounter < iSeriesLength; iSeriesCounter += 1) {
                    ...
                    if (isSeriesEnabled(oSeries, iSeriesCounter)) {
                        ...
                        for (iCounter = 0; iCounter < iLength; iCounter += 1) {
                            ...
                            retain = oRectAttr.x;
                            if (chartConfigOptions.series[iSeriesCounter].childenabled) {
                                oRectAttr.x -= ((iSeriesCounter + 1) / 2) * oBarWidth;
                                oRectAttr.x = oRectAttr.x - fGrpPadding + (oBarWidth / 8);
                                oRectAttr.width = (oBarWidth * 0.75);
                                oRect = MAQ.createSVGElement(chartConfigOptions.svgNS, 'rect', oRectAttr);
                                oGrpColumnChart.appendChild(oRect);
                                oRectAttr.x = retain;
                            } else {
                                oRectAttr.x -= printedChild * oBarWidth
                                oRect = MAQ.createSVGElement(chartConfigOptions.svgNS, 'rect', oRectAttr);
                                oGrpColumnChart.appendChild(oRect);
                            }
                            ...
                            if (oSeries[iSeriesCounter].data[iCounter] === "" || !oSeries[iSeriesCounter].data[iCounter]) {
        ...
        */
        MAQ.createBarChart = function (chartConfigOptions) {
            ...
            if (oDataInfo.min || oDataInfo.max) {
                ...
                fGrpPadding = fGrpPadding / 10 * oBarWidth;
                if (!oColumnPlotOptions.stacked) {
                    oBarWidth = oBarWidth - fGrpPadding * (iSeriesLength - 1);
                }
